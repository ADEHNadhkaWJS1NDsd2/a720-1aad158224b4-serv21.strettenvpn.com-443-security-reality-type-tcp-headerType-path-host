...
local function BuildRuntime()
    ...
    local function CreateDropdown(Section, Name, Values, Default, Flag, Options)
        ...
        local function RefreshClosedState(Hovered)
            if IsOpened then
                Button.BackgroundColor3 = Color3.fromRGB(17, 28, 36)
                ButtonStroke.Color = Accent
                Divider.BackgroundTransparency = 0.15
                Arrow.ImageColor3 = PrimaryText
            else
                Button.BackgroundColor3 = Hovered and Color3.fromRGB(17, 28, 36) or SurfaceAlt
                ButtonStroke.Color = Hovered and Color3.fromRGB(40, 64, 78) or Border
        ...
        Bind(Button.MouseButton1Click:Connect(function()
            ...
            Stroke(ActivePopup, Border, 0.10, 1)
            Menu:AddSoftGlow(ActivePopup, 100, 7, 0.84, true)
            ...
            for _, Value in ipairs(Values) do
                ...
                local Option = Create("TextButton", {
                    ...
                    BackgroundColor3 = Selected and Color3.fromRGB(17, 28, 36) or Surface,
    ...
    local function CreateMultiDropdown(Section, Name, Values, Default, Flag, Options)
        ...
        Bind(Button.MouseButton1Click:Connect(function()
            ...
            Menu.PopupInputBlocker.Visible = true
            local Width = 128
            local VisibleRows = math.min(#Values, 8)
            local PopupHeight = math.max(VisibleRows * 24 + 12, 36)
            ActivePopup = Create("Frame", {
                Parent = ScreenGui,
                Active = true,
                Position = UDim2.fromOffset(Button.AbsolutePosition.X + Button.AbsoluteSize.X - Width, Button.AbsolutePosition.Y + Button.AbsoluteSize.Y + 6),
            ...
            local List = Create("ScrollingFrame", {
                ...
                ScrollBarThickness = #Values > 8 and 2 or 0,
                ScrollBarImageColor3 = Accent,
    ...
    local function CreateEspPreviewWindow()
        ...
        local PreviewLayoutFlags = {
            HealthBarSide = "Player ESP Health Bar Side",
            HealthValueSide = "Player ESP Health Value Side",
            HealthValuePosition = "Player ESP Health Value Position"
        }
        ...
        local function SetPreviewOffset(Key, Value)
            ...
            if Key == "HealthBar" then
                Menu.Flags[PreviewLayoutFlags.HealthBarSide] =
                    Value.X > 0 and "Right" or "Left"
            elseif Key == "HealthValue" then
                Menu.Flags[PreviewLayoutFlags.HealthValueSide] =
                    Value.X > 0 and "Right" or "Left"

                Menu.Flags[PreviewLayoutFlags.HealthValuePosition] =
                    Value.Y
            end
        end

        if typeof(Menu.Flags[PreviewElementFlags.HealthBar]) ~= "Vector2" then
            Menu.Flags[PreviewElementFlags.HealthBar] = Vector2.zero
        end

        if typeof(Menu.Flags[PreviewElementFlags.HealthValue]) ~= "Vector2" then
            Menu.Flags[PreviewElementFlags.HealthValue] = Vector2.zero
        end

        if typeof(Menu.Flags[PreviewElementFlags.Name]) ~= "Vector2" then
            Menu.Flags[PreviewElementFlags.Name] = Vector2.zero
        end

        if typeof(Menu.Flags[PreviewElementFlags.Weapon]) ~= "Vector2" then
            Menu.Flags[PreviewElementFlags.Weapon] = Vector2.zero
        end

        if typeof(Menu.Flags[PreviewElementFlags.Distance]) ~= "Vector2" then
            Menu.Flags[PreviewElementFlags.Distance] = Vector2.zero
        end

        if Menu.Flags[PreviewLayoutFlags.HealthBarSide] == nil then
            Menu.Flags[PreviewLayoutFlags.HealthBarSide] = "Left"
        end

        if Menu.Flags[PreviewLayoutFlags.HealthValueSide] == nil then
            Menu.Flags[PreviewLayoutFlags.HealthValueSide] = "Left"
        end

        if Menu.Flags[PreviewLayoutFlags.HealthValuePosition] == nil then
            Menu.Flags[PreviewLayoutFlags.HealthValuePosition] = 0
        end
    ...
    function Library:SetNotificationLayout(Position, Scale)
        ...
        local AX = Horizontal == "Left" and 0 or Horizontal == "Center" and 0.5 or 1
        local Y = Vertical == "Top" and 0.045 or Vertical == "Middle" and 0.5 or 0.955
        local AY = Vertical == "Top" and 0 or Vertical == "Middle" and 0.5 or 1
        ...
        if Layout then
            ...
            Layout.HorizontalAlignment = Horizontal == "Left" and Enum.HorizontalAlignment.Left or Horizontal == "Center" and Enum.HorizontalAlignment.Center or Enum.HorizontalAlignment.Right
            Layout.VerticalAlignment = Vertical == "Top" and Enum.VerticalAlignment.Top or Vertical == "Middle" and Enum.VerticalAlignment.Center or Enum.VerticalAlignment.Bottom
        end
    ...
    function Library:Notification(TitleOrData, DescriptionOrDuration, DurationOrColor, ModeValue)
        ...
        local ModeColor = NormalizedMode == "danger" and Color3.fromRGB(235, 78, 88)
            or NormalizedMode == "warn" and Color3.fromRGB(224, 174, 70)
            or Color3.fromRGB(83, 202, 132)
        ...
        Corner(Notice, 4)
        local NoticeStroke = Stroke(Notice, Color3.fromRGB(35, 39, 45), 0.12, 1)
