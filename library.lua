...
function LibraryApi:CreateWindow(windowName)
    ...
    function windowContext:Tab_Create(tabName, iconId)
        ...
        local function activate()
            ...
            td.indicator.Position = Vector2.new(WX + 6, ty + 8)
        ...
        userInputService.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                if isMouseOver(WX + 6, ty, sideW - 12, 30) and windowVisible then
                    activate()
                end
            end
        end)
        ...
        runService.RenderStepped:Connect(function()
            ...
            if isMouseOver(WX + 6, ty, sideW - 12, 30) then
    ...
    function windowContext:Tab_Create(tabName, iconId)
        return self:Tab_Create(tabName, iconId)
    end

    local realTabCreate = windowContext.Tab_Create

    windowContext.Tab_Create = function(self, tabName, iconId)
        local tabIndex = #self.Tabs + 1
        local ty = WY + topH + 8 + (tabIndex - 1) * 36
        ...
        local td = {}
        td.hoverBg = drawRect(WX + 6, ty, sideW - 12, 30, colors.elementHoverBackground, 1, 3, true)
        ...
        local function activate()
            if self.Active_Tab == tabData then return end
            if self.Active_Tab then
                local old = self.Active_Tab
                old._drawings.label.Color = colors.textDarkColor
                old._drawings.indicator.Size = Vector2.new(2, 0)
                for _, eg in ipairs(old._elements) do
                    for _, v in pairs(eg) do
                        if type(v) == "table" and v.Visible ~= nil then v.Visible = false end
                    end
                end
            end
            self.Active_Tab = tabData
            td.label.Color = colors.textWhiteColor
            td.indicator.Size = Vector2.new(2, 14)
            for _, eg in ipairs(tabData._elements) do
                for _, v in pairs(eg) do
                    if type(v) == "table" and v.Visible ~= nil then v.Visible = true end
                end
            end
        end
        ...
        local function isVisible()
            return self.Active_Tab == tabData and windowVisible
        end
        ...
        local function elementInjector(side)
            ...
            local function getCX()
                return side == "Left" and columnLeftX or columnRightX
            end
            local function getCY()
                return side == "Left" and leftCursorY or rightCursorY
            end
            local function advance(amount)
                if side == "Left" then leftCursorY = leftCursorY + amount
                else rightCursorY = rightCursorY + amount end
            end
            ...
            function elements:Toggle_Create(name, flag, default, tooltip, callback)
                ...
                local dg = {}
                dg.cb = drawRect(cx + 2, cy + 2, 13, 13, LibraryApi.Flags[flag] and colors.accentColor or colors.elementBackground, 0.78, 10, true)
                dg.cbB = drawRect(cx + 2, cy + 2, 13, 13, LibraryApi.Flags[flag] and colors.accentColor or colors.borderColor, 1, 10, false, 1)
                dg.lbl = drawText(name, cx + 22, cy + 3, 12, LibraryApi.Flags[flag] and colors.textWhiteColor or colors.textDarkColor, 1, 10)
                ...
                userInputService.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 and windowVisible and self.Active_Tab == tabData then
                        if isMouseOver(cx, lcy, sw, h) then
                            LibraryApi.Flags[flag] = not LibraryApi.Flags[flag]
                            local ns = LibraryApi.Flags[flag]
                            dg.cb.Color = ns and colors.accentColor or colors.elementBackground
                            dg.cbB.Color = ns and colors.accentColor or colors.borderColor
                            dg.lbl.Color = ns and colors.textWhiteColor or colors.textDarkColor
                            saveConfiguration()
                            if callback then task.spawn(callback, ns) end
                        end
                    end
                ...
                runService.RenderStepped:Connect(function()
                    ...
                    if isMouseOver(cx, lcy, sw, h) then showTooltip(tooltip)
                    elseif not LibraryApi.Flags[flag] then dg.cbB.Color = colors.borderColor end
                end)
            end
            ...
            function elements:Slider_Create(name, flag, min, max, default, step, tooltip, callback)
                ...
                dg.val = drawText(formatValue(LibraryApi.Flags[flag], step), cx + sw - 2, cy + 1, 12, colors.textWhiteColor, 1, 10)
                dg.track = drawRect(cx + 2, cy + 18, sw - 4, 6, colors.elementBackground, 0.78, 10, true)
                dg.trackB = drawRect(cx + 2, cy + 18, sw - 4, 6, colors.borderColor, 1, 10, false, 1)
                local pct = (LibraryApi.Flags[flag] - min) / (max - min)
                dg.fill = drawRect(cx + 2, cy + 18, math.max(1, (sw - 4) * pct), 6, colors.accentColor, 1, 11, true)
                dg.knob = drawCircle(cx + 2 + (sw - 4) * pct, cy + 21, 5, colors.textWhiteColor, 1, 12, true)
                dg.knobB = drawCircle(cx + 2 + (sw - 4) * pct, cy + 21, 5, colors.borderColor, 1, 12, false, 1)
                for _, v in pairs(dg) do v.Visible = isVisible() end
                for _, v in pairs(dg) do table.insert(elementGroup, v) end
                advance(40)
                ...
                local function setVal(newVal)
                    ...
                    LibraryApi.Flags[flag] = clamped
                    local p = (clamped - min) / (max - min)
                    dg.fill.Size = Vector2.new(math.max(1, (sw - 4) * p), 6)
                    dg.knob.Position = Vector2.new(cx + 2 + (sw - 4) * p, lcy + 21)
                    dg.knobB.Position = dg.knob.Position
                    dg.val.Text = formatValue(clamped, step)
                    saveConfiguration()
                    if callback then task.spawn(callback, clamped) end
                end
                userInputService.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 and windowVisible and self.Active_Tab == tabData then
                        if isMouseOver(cx + 2, lcy + 15, sw - 4, 12) then
                            isSliding = true
                            local p = math.clamp((userInputService:GetMouseLocation().X - (cx + 2)) / (sw - 4), 0, 1)
                            setVal(min + (max - min) * p)
                        end
                    end
                end)
                ...
                userInputService.InputChanged:Connect(function(input)
                    if isSliding and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                        local p = math.clamp((input.Position.X - (cx + 2)) / (sw - 4), 0, 1)
                        setVal(min + (max - min) * p)
                    end
                end)
                runService.RenderStepped:Connect(function()
                    ...
                    if isMouseOver(cx + 2, lcy + 15, sw - 4, 12) then
                        showTooltip(tooltip)
                        dg.trackB.Color = colors.borderLightColor
                    else
                        dg.trackB.Color = colors.borderColor
                    end
                end)
            end
            ...
            function elements:RangeSlider_Create(name, flag, min, max, defaultMin, defaultMax, step, tooltip, callback)
                ...
                local dg = {}
                dg.lbl = drawText(name, cx + 2, cy + 1, 12, colors.textWhiteColor, 1, 10)
                dg.val = drawText(formatValue(LibraryApi.Flags[flag].Min, step) .. " - " .. formatValue(LibraryApi.Flags[flag].Max, step), cx + sw - 2, cy + 1, 12, colors.textWhiteColor, 1, 10)
                dg.track = drawRect(cx + 2, cy + 18, sw - 4, 6, colors.elementBackground, 0.78, 10, true)
                dg.trackB = drawRect(cx + 2, cy + 18, sw - 4, 6, colors.borderColor, 1, 10, false, 1)
                local mnP = (LibraryApi.Flags[flag].Min - min) / (max - min)
                local mxP = (LibraryApi.Flags[flag].Max - min) / (max - min)
                dg.fill = drawRect(cx + 2 + (sw - 4) * mnP, cy + 18, (sw - 4) * (mxP - mnP), 6, colors.accentColor, 1, 11, true)
                dg.minK = drawCircle(cx + 2 + (sw - 4) * mnP, cy + 21, 5, colors.textWhiteColor, 1, 12, true)
                dg.maxK = drawCircle(cx + 2 + (sw - 4) * mxP, cy + 21, 5, colors.textWhiteColor, 1, 12, true)
                dg.minKB = drawCircle(cx + 2 + (sw - 4) * mnP, cy + 21, 5, colors.borderColor, 1, 12, false, 1)
                dg.maxKB = drawCircle(cx + 2 + (sw - 4) * mxP, cy + 21, 5, colors.borderColor, 1, 12, false, 1)
                for _, v in pairs(dg) do v.Visible = isVisible() end
                for _, v in pairs(dg) do table.insert(elementGroup, v) end
                advance(40)
                local lcy = cy
                ...
                local function updateRV()
                    ...
                    local mx2 = (LibraryApi.Flags[flag].Max - min) / (max - min)
                    dg.fill.Position = Vector2.new(cx + 2 + (sw - 4) * mn2, lcy + 18)
                    dg.fill.Size = Vector2.new(math.max(1, (sw - 4) * (mx2 - mn2)), 6)
                    dg.minK.Position = Vector2.new(cx + 2 + (sw - 4) * mn2, lcy + 21)
                    dg.minKB.Position = dg.minK.Position
                    dg.maxK.Position = Vector2.new(cx + 2 + (sw - 4) * mx2, lcy + 21)
                    dg.maxKB.Position = dg.maxK.Position
                    dg.val.Text = formatValue(LibraryApi.Flags[flag].Min, step) .. " - " .. formatValue(LibraryApi.Flags[flag].Max, step)
                end
                userInputService.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 and windowVisible and self.Active_Tab == tabData then
                        if isMouseOver(cx + 2, lcy + 15, sw - 4, 12) then
                            ...
                            local mn2 = (LibraryApi.Flags[flag].Min - min) / (max - min)
                            local mx2 = (LibraryApi.Flags[flag].Max - min) / (max - min)
                            if math.abs(p - mn2) < math.abs(p - mx2) then isSlMin = true else isSlMax = true end
                        end
                    end
                end)
                userInputService.InputEnded:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then isSlMin = false isSlMax = false end
                end)
                userInputService.InputChanged:Connect(function(input)
                    if (isSlMin or isSlMax) and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                        local p = math.clamp((input.Position.X - (cx + 2)) / (sw - 4), 0, 1)
                        local v2 = snapValue(min + (max - min) * p, step)
                        if isSlMin then LibraryApi.Flags[flag].Min = math.min(v2, LibraryApi.Flags[flag].Max)
                        else LibraryApi.Flags[flag].Max = math.max(v2, LibraryApi.Flags[flag].Min) end
                        updateRV()
                        saveConfiguration()
                        if callback then task.spawn(callback, LibraryApi.Flags[flag]) end
                    end
            ...
            function elements:Textbox_Create(name, flag, default, tooltip, callback)
                ...
                local inputText = LibraryApi.Flags[flag]
                local isFocused = false
                local cursorVis = true
                local cursorT = 0
                local dg = {}
                dg.lbl = drawText(name, cx + 2, cy + 1, 12, colors.textWhiteColor, 1, 10)
                dg.inputBg = drawRect(cx + 2, cy + 18, sw - 4, 20, colors.elementBackground, 0.78, 10, true)
                dg.inputB = drawRect(cx + 2, cy + 18, sw - 4, 20, colors.borderColor, 1, 10, false, 1)
                dg.inputTxt = drawText(inputText, cx + 6, cy + 22, 11, colors.textDarkColor, 1, 11)
                dg.cursor = drawRect(cx + 6 + getTextSize(inputText, 11), cy + 20, 1, 14, colors.textWhiteColor, 0, 11, true)
                for _, v in pairs(dg) do v.Visible = isVisible() end
                for _, v in pairs(dg) do table.insert(elementGroup, v) end
                advance(42)
                local lcy = cy
                userInputService.InputBegan:Connect(function(input)
                    if not windowVisible or self.Active_Tab ~= tabData then return end
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if isMouseOver(cx + 2, lcy + 18, sw - 4, 20) then
                            isFocused = true
                            dg.inputB.Color = colors.accentColor
                            dg.inputTxt.Color = colors.textWhiteColor
                        elseif isFocused then
                            isFocused = false
                            dg.inputB.Color = colors.borderColor
                            dg.inputTxt.Color = colors.textDarkColor
                            dg.cursor.Transparency = 0
                            LibraryApi.Flags[flag] = inputText
                            saveConfiguration()
                            if callback then task.spawn(callback, inputText) end
                        end
                    ...
                    if isFocused and input.UserInputType == Enum.UserInputType.Keyboard then
                        ...
                        dg.inputTxt.Text = inputText
                        local tw = getTextSize(inputText, 11)
                        dg.cursor.Position = Vector2.new(cx + 6 + tw, lcy + 20)
                    end
                end)
                userInputService.InputChanged:Connect(function(input)
                    if isFocused and input.UserInputType == Enum.UserInputType.TextInput then
                        ...
                        if char >= 32 and char < 127 then
                            inputText = inputText .. string.char(char)
                            dg.inputTxt.Text = inputText
                            dg.cursor.Position = Vector2.new(cx + 6 + getTextSize(inputText, 11), lcy + 20)
                        end
                    end
                end)
                runService.RenderStepped:Connect(function(dt)
                    if isFocused then
                        cursorT = cursorT + dt
                        if cursorT >= 0.5 then cursorT = 0 cursorVis = not cursorVis dg.cursor.Transparency = cursorVis and 1 or 0 end
                    end
                    if not windowVisible or self.Active_Tab ~= tabData then return end
                    if isMouseOver(cx + 2, lcy + 18, sw - 4, 20) then
                        showTooltip(tooltip)
                        if not isFocused then dg.inputB.Color = colors.borderLightColor end
                    else
                        if not isFocused then dg.inputB.Color = colors.borderColor end
                    end
                end)
            end
            ...
            function elements:Keybind_Create(name, flag, default, tooltip, callback)
                ...
                local dg = {}
                dg.lbl = drawText(name, cx + 2, cy + 7, 12, colors.textWhiteColor, 1, 10)
                dg.btnBg = drawRect(cx + sw - 72, cy + 3, 70, 22, colors.elementBackground, 0.78, 10, true)
                dg.btnB = drawRect(cx + sw - 72, cy + 3, 70, 22, colors.borderColor, 1, 10, false, 1)
                dg.btnT = drawText(getKT(), cx + sw - 72 + 35, cy + 8, 11, colors.textDarkColor, 1, 11)
                dg.btnT.Center = true
                for _, v in pairs(dg) do v.Visible = isVisible() end
                for _, v in pairs(dg) do table.insert(elementGroup, v) end
                advance(32)
                ...
                userInputService.InputBegan:Connect(function(input)
                    if not windowVisible or self.Active_Tab ~= tabData then return end
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if isMouseOver(cx + sw - 72, lcy + 3, 70, 22) then
                            isListening = true
                            dg.btnT.Text = "[ ... ]"
                            dg.btnB.Color = colors.accentColor
                            dg.btnT.Color = colors.textWhiteColor
                        end
                    end
                    if isListening and input.UserInputType == Enum.UserInputType.Keyboard then
                        if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Enum.KeyCode.Escape then
                        ...
                        isListening = false
                        dg.btnT.Text = getKT()
                        dg.btnB.Color = colors.borderColor
                        dg.btnT.Color = colors.textDarkColor
                        saveConfiguration()
                        if callback then task.spawn(callback, LibraryApi.Flags[flag]) end
                    elseif not isListening and input.UserInputType == Enum.UserInputType.Keyboard then
                        if input.KeyCode == LibraryApi.Flags[flag] and input.KeyCode ~= Enum.KeyCode.Unknown then
                ...
                runService.RenderStepped:Connect(function()
                    ...
                    if isMouseOver(cx + sw - 72, lcy + 3, 70, 22) then
                        showTooltip(tooltip)
                        if not isListening then dg.btnB.Color = colors.borderLightColor end
                    else
                        if not isListening then dg.btnB.Color = colors.borderColor end
                    end
                end)
            end
            ...
            function elements:Dropdown_Create(name, flag, options, default, tooltip, callback)
                ...
                local dg = {}
                dg.lbl = drawText(name, cx + 2, cy + 1, 12, colors.textWhiteColor, 1, 10)
                dg.btnBg = drawRect(cx + 2, cy + 16, sw - 4, 22, colors.elementBackground, 0.78, 10, true)
                dg.btnB = drawRect(cx + 2, cy + 16, sw - 4, 22, colors.borderColor, 1, 10, false, 1)
                dg.sel = drawText(LibraryApi.Flags[flag], cx + 8, cy + 21, 12, colors.textDarkColor, 1, 11)
                dg.arr = drawText("v", cx + sw - 10, cy + 21, 11, colors.textDarkColor, 1, 11)
                local optDs = {}
                for i, opt in ipairs(options) do
                    local oy = cy + baseH + (i - 1) * optH
                    local od = {}
                    od.bg = drawRect(cx + 2, oy, sw - 4, optH, colors.elementBackground, 0, 12, true)
                    od.bg.Visible = false
                    od.txt = drawText(opt, cx + 8, oy + 5, 12, LibraryApi.Flags[flag] == opt and colors.accentColor or colors.textDarkColor, 0, 13)
                    od.txt.Visible = false
                    optDs[i] = od
                    table.insert(elementGroup, od.bg)
                    table.insert(elementGroup, od.txt)
                end
                for _, v in pairs(dg) do v.Visible = isVisible() end
                for _, v in pairs(dg) do table.insert(elementGroup, v) end
                advance(baseH + 4)
                ...
                local function toggleOpen2()
                    ...
                    dg.arr.Text = isOpen and "^" or "v"
                    for _, od in ipairs(optDs) do
                        local show = isOpen and isVisible()
                        od.bg.Visible = show
                        od.txt.Visible = show
                        od.bg.Transparency = show and 0.78 or 0
                        od.txt.Transparency = show and 1 or 0
                    end
                end
                userInputService.InputBegan:Connect(function(input)
                    if not windowVisible or self.Active_Tab ~= tabData then return end
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if isMouseOver(cx + 2, lcy + 16, sw - 4, 22) then
                            toggleOpen2()
                        elseif isOpen then
                            for i, opt in ipairs(options) do
                                local oy = lcy + baseH + (i - 1) * optH
                                if isMouseOver(cx + 2, oy, sw - 4, optH) then
                                    LibraryApi.Flags[flag] = opt
                                    dg.sel.Text = opt
                                    for j, od in ipairs(optDs) do
                                        od.txt.Color = j == i and colors.accentColor or colors.textDarkColor
                                    end
                                    toggleOpen2()
                                    saveConfiguration()
                                    if callback then task.spawn(callback, opt) end
                ...
                runService.RenderStepped:Connect(function()
                    ...
                    if isMouseOver(cx + 2, lcy + 16, sw - 4, 22) then
                        showTooltip(tooltip)
                        if not isOpen then dg.btnB.Color = colors.borderLightColor end
                    else
                        if not isOpen then dg.btnB.Color = colors.borderColor end
                    end
                end)
            end
            ...
            function elements:ColorPicker_Create(name, flag, default, tooltip, callback)
                ...
                local hueBarH = 12
                local dg = {}
                dg.lbl = drawText(name, cx + 2, cy + 4, 12, colors.textWhiteColor, 1, 10)
                dg.prev = drawRect(cx + sw - 28, cy, 24, 16, LibraryApi.Flags[flag], 1, 10, true)
                dg.prevB = drawRect(cx + sw - 28, cy, 24, 16, colors.borderColor, 1, 10, false, 1)
                local eY = cy + 22
                dg.pBg = drawRect(cx + 2, eY, pickerW, pickerH, colors.elementBackground, 0, 10, true)
                dg.pBg.Visible = false
                dg.pB = drawRect(cx + 2, eY, pickerW, pickerH, colors.borderColor, 0, 10, false, 1)
                dg.pB.Visible = false
                dg.svBg = drawRect(cx + 4, eY + 4, pickerW - 8, pickerH - 24, Color3.fromHSV(h2, 1, 1), 0, 11, true)
                dg.svBg.Visible = false
                local svW2 = pickerW - 8
                local svH2 = pickerH - 24
                dg.svC = drawCircle(cx + 4 + svW2 * s2, eY + 4 + svH2 * (1 - v2), 4, Color3.new(1,1,1), 0, 14, true)
                dg.svC.Visible = false
                dg.svCB = drawCircle(cx + 4 + svW2 * s2, eY + 4 + svH2 * (1 - v2), 4, Color3.new(0,0,0), 0, 14, false, 1)
                dg.svCB.Visible = false
                local hY2 = eY + pickerH - 16
                dg.hBar = drawRect(cx + 4, hY2, pickerW - 8, hueBarH, Color3.new(1,1,1), 0, 11, true)
                dg.hBar.Visible = false
                dg.hBarB = drawRect(cx + 4, hY2, pickerW - 8, hueBarH, colors.borderColor, 0, 11, false, 1)
                dg.hBarB.Visible = false
                dg.hC = drawRect(cx + 4 + (pickerW - 8) * h2 - 2, hY2 - 2, 4, hueBarH + 4, Color3.new(1,1,1), 0, 12, true)
                dg.hC.Visible = false
                dg.hCB = drawRect(cx + 4 + (pickerW - 8) * h2 - 2, hY2 - 2, 4, hueBarH + 4, Color3.new(0,0,0), 0, 12, false, 1)
                dg.hCB.Visible = false
                for _, v in pairs(dg) do table.insert(elementGroup, v) end
                for _, v in pairs(dg) do if v.Visible ~= false then v.Visible = isVisible() end end
                dg.lbl.Visible = isVisible()
                dg.prev.Visible = isVisible()
                ...
                local function upC()
                    ...
                    LibraryApi.Flags[flag] = c
                    dg.prev.Color = c
                    dg.svBg.Color = Color3.fromHSV(h2, 1, 1)
                    dg.svC.Position = Vector2.new(cx + 4 + svW2 * s2, eY + 4 + svH2 * (1 - v2))
                    dg.svCB.Position = dg.svC.Position
                    dg.hC.Position = Vector2.new(cx + 4 + (pickerW - 8) * h2 - 2, hY2 - 2)
                    dg.hCB.Position = dg.hC.Position
                    saveConfiguration()
                    if callback then task.spawn(callback, c) end
                end
                userInputService.InputBegan:Connect(function(input)
                    if not windowVisible or self.Active_Tab ~= tabData then return end
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if isMouseOver(cx + sw - 28, lcy, 24, 16) then
                            isOpen = not isOpen
                            local show = isOpen and isVisible()
                            dg.pBg.Visible = show dg.pBg.Transparency = show and 0.78 or 0
                            dg.pB.Visible = show dg.pB.Transparency = show and 1 or 0
                            dg.svBg.Visible = show dg.svBg.Transparency = show and 1 or 0
                            dg.svC.Visible = show dg.svC.Transparency = show and 1 or 0
                            dg.svCB.Visible = show dg.svCB.Transparency = show and 1 or 0
                            dg.hBar.Visible = show dg.hBar.Transparency = show and 1 or 0
                            dg.hBarB.Visible = show dg.hBarB.Transparency = show and 1 or 0
                            dg.hC.Visible = show dg.hC.Transparency = show and 1 or 0
                            dg.hCB.Visible = show dg.hCB.Transparency = show and 1 or 0
                            dg.prevB.Color = isOpen and colors.accentColor or colors.borderColor
                        elseif isOpen then
                            local mx, my = input.Position.X, input.Position.Y
                            if isMouseOver(cx + 4, eY + 4, svW2, svH2) then
                                isSV = true
                                s2 = math.clamp((mx - (cx + 4)) / svW2, 0, 1)
                                v2 = 1 - math.clamp((my - (eY + 4)) / svH2, 0, 1)
                                upC()
                            elseif isMouseOver(cx + 4, hY2, pickerW - 8, hueBarH) then
                                isH = true
                                h2 = math.clamp((mx - (cx + 4)) / (pickerW - 8), 0, 1)
                                upC()
                            end
                        end
                    end
                ...
                userInputService.InputEnded:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then isSV = false isH = false end
                end)
                userInputService.InputChanged:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
                        local mx, my = input.Position.X, input.Position.Y
                ...
                runService.RenderStepped:Connect(function()
                    ...
                    if isMouseOver(cx + sw - 28, lcy, 24, 16) then
                        showTooltip(tooltip)
                        if not isOpen then dg.prevB.Color = colors.borderLightColor end
                    else
                        if not isOpen then dg.prevB.Color = colors.borderColor end
                    end
                end)
            end
            ...
            function elements:Button_Create(name, tooltip, callback)
                ...
                local h3 = 28
                local dg = {}
                dg.bg = drawRect(cx + 2, cy, sw - 4, h3, colors.elementBackground, 0.78, 10, true)
                dg.brd = drawRect(cx + 2, cy, sw - 4, h3, colors.borderColor, 1, 10, false, 1)
                dg.txt = drawText(name, cx + 2 + (sw - 4) / 2, cy + 8, 12, colors.textWhiteColor, 1, 11)
                dg.txt.Center = true
                for _, v in pairs(dg) do v.Visible = isVisible() end
                for _, v in pairs(dg) do table.insert(elementGroup, v) end
                advance(h3 + 6)
                ...
                userInputService.InputBegan:Connect(function(input)
                    if not windowVisible or self.Active_Tab ~= tabData then return end
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if isMouseOver(cx + 2, lcy, sw - 4, h3) then
                            isDown = true
                            dg.bg.Size = Vector2.new((sw - 4) * 0.96, h3 * 0.85)
                            dg.bg.Position = Vector2.new(cx + 2 + (sw - 4) * 0.02, lcy + h3 * 0.075)
                        end
                    end
                end)
                userInputService.InputEnded:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 and isDown then
                        isDown = false
                        dg.bg.Size = Vector2.new(sw - 4, h3)
                        dg.bg.Position = Vector2.new(cx + 2, lcy)
                        if isMouseOver(cx + 2, lcy, sw - 4, h3) and callback then task.spawn(callback) end
                    end
                end)
                runService.RenderStepped:Connect(function()
                    if not windowVisible or self.Active_Tab ~= tabData then return end
                    if isMouseOver(cx + 2, lcy, sw - 4, h3) then
                        showTooltip(tooltip)
                        dg.bg.Color = colors.elementHoverBackground
                        dg.brd.Color = colors.accentColor
                        dg.txt.Color = colors.accentColor
                    else
                        dg.bg.Color = colors.elementBackground
                        dg.brd.Color = colors.borderColor
                        dg.txt.Color = colors.textWhiteColor
                    end
                end)
            end
            ...
            function elements:SubButton_Create(name, tooltip, callback)
                ...
                local h4 = 20
                local dg = {}
                dg.bg = drawRect(cx + 8, cy, sw, h4, colors.sectionBackground, 0.78, 10, true)
                dg.brd = drawRect(cx + 8, cy, sw, h4, colors.borderColor, 1, 10, false, 1)
                dg.txt = drawText(name, cx + 8 + sw / 2, cy + 4, 11, colors.textDarkColor, 1, 11)
                dg.txt.Center = true
                for _, v in pairs(dg) do v.Visible = isVisible() end
                for _, v in pairs(dg) do table.insert(elementGroup, v) end
                advance(h4 + 4)
                ...
                userInputService.InputBegan:Connect(function(input)
                    if not windowVisible or self.Active_Tab ~= tabData then return end
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if isMouseOver(cx + 8, lcy, sw, h4) then isDown = true end
                    end
                end)
                userInputService.InputEnded:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 and isDown then
                        isDown = false
                        if isMouseOver(cx + 8, lcy, sw, h4) and callback then task.spawn(callback) end
                    end
                end)
                runService.RenderStepped:Connect(function()
                    if not windowVisible or self.Active_Tab ~= tabData then return end
                    if isMouseOver(cx + 8, lcy, sw, h4) then
                        showTooltip(tooltip)
                        dg.bg.Color = colors.elementBackground
                        dg.brd.Color = colors.borderLightColor
                        dg.txt.Color = colors.textWhiteColor
                    else
                        dg.bg.Color = colors.sectionBackground
                        dg.brd.Color = colors.borderColor
                        dg.txt.Color = colors.textDarkColor
                    end
                end)
            end
            ...
            function elements:Module_Create(name, flag, descriptionText, default, tooltip, callback)
                ...
                local h5 = 44
                local dg = {}
                dg.bg = drawRect(cx + 2, cy, sw - 4, h5, colors.elementBackground, 0.78, 10, true)
                dg.brd = drawRect(cx + 2, cy, sw - 4, h5, LibraryApi.Flags[flag] and colors.accentColor or colors.borderColor, 1, 10, false, 1)
                dg.cb = drawRect(cx + 14, cy + 14, 16, 16, LibraryApi.Flags[flag] and colors.accentColor or colors.sectionBackground, 0.78, 11, true)
                dg.cbB = drawRect(cx + 14, cy + 14, 16, 16, colors.borderColor, 1, 11, false, 1)
                dg.name = drawText(name, cx + 38, cy + 8, 13, LibraryApi.Flags[flag] and colors.textWhiteColor or colors.textDarkColor, 1, 11)
                dg.desc = drawText(descriptionText or "", cx + 38, cy + 24, 11, colors.textDarkColor, 1, 11)
                dg.arr = drawText(LibraryApi.Flags[flag] and "^" or "v", cx + sw - 22, cy + 15, 11, LibraryApi.Flags[flag] and colors.accentColor or colors.textDarkColor, 1, 11)
                for _, v in pairs(dg) do v.Visible = isVisible() end
                for _, v in pairs(dg) do table.insert(elementGroup, v) end
                advance(h5 + 6)
                local lcy = cy
                userInputService.InputBegan:Connect(function(input)
                    if not windowVisible or self.Active_Tab ~= tabData then return end
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if isMouseOver(cx + 2, lcy, sw - 4, h5) then
                            LibraryApi.Flags[flag] = not LibraryApi.Flags[flag]
                            local ns = LibraryApi.Flags[flag]
                            dg.brd.Color = ns and colors.accentColor or colors.borderColor
                            dg.cb.Color = ns and colors.accentColor or colors.sectionBackground
                            dg.name.Color = ns and colors.textWhiteColor or colors.textDarkColor
                            dg.arr.Text = ns and "^" or "v"
                            dg.arr.Color = ns and colors.accentColor or colors.textDarkColor
                            saveConfiguration()
                            if callback then task.spawn(callback, ns) end
                        end
                    end
                ...
                runService.RenderStepped:Connect(function()
                    ...
                    if isMouseOver(cx + 2, lcy, sw - 4, h5) then
                        showTooltip(tooltip)
                        if not LibraryApi.Flags[flag] then dg.brd.Color = colors.borderLightColor end
                    else
                        if not LibraryApi.Flags[flag] then dg.brd.Color = colors.borderColor end
                    end
                end)
                return elementInjector(side)
            end
        ...
        function sectionApi:Section_Create(columnSide, sectionTitle)
            ...
            local titleH = 24
            local dg = {}
            dg.bg = drawRect(cx, cy, sw, titleH + 8, colors.sectionBackground, 0.78, 8, true)
            dg.brd = drawRect(cx, cy, sw, titleH + 8, colors.borderColor, 1, 8, false, 1)
            ...
            local secStartY = columnSide == "Left" and leftCursorY or rightCursorY
            local secElements = elementInjector(columnSide)

            runService.RenderStepped:Connect(function()
                local endY = columnSide == "Left" and leftCursorY or rightCursorY
                local totalH = titleH + 8 + math.max(0, endY - secStartY)
                dg.bg.Size = Vector2.new(sw, totalH)
                dg.brd.Size = Vector2.new(sw, totalH)
            end)
