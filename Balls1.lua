...
function Library:Tween(instance, properties, duration, style)
	...
	local active = Library.ActiveTweens[instance]
	if not active then
		active = {}
		Library.ActiveTweens[instance] = active
	end

	local cancelled = {}
	for property in properties do
		local previous = active[property]
	...
	local tween = TweenService:Create(instance, TweenInfo.new(seconds, easing, Enum.EasingDirection.Out), properties)
	for property in properties do
		active[property] = tween
	end
	tween:Play()
	return tween
end
...
function Utils.Round(value, step)
	...
	local scaled, decimals = step, 0
	while decimals < 8 and math.abs(scaled - math.floor(scaled + 0.5)) > 1e-9 do
		scaled *= 10
		decimals += 1
	end
...
function Utils.ToHex(color)
	local r = math.floor(Utils.Clamp(color.R, 0, 1) * 255 + 0.5)
	local g = math.floor(Utils.Clamp(color.G, 0, 1) * 255 + 0.5)
	local b = math.floor(Utils.Clamp(color.B, 0, 1) * 255 + 0.5)
...
function Utils.BindGradient(gradient, topKey, bottomKey)
	return Library:Paint(gradient, "Color", function()
		return ColorSequence.new(Themes.Current[topKey], Themes.Current[bottomKey])
	end, { topKey, bottomKey })
end
...
function Library:RepaintKeys(keys)
	...
	-- Multi-key case: deduplicate without allocating a table on every repaint.
	Library.RepaintSerial = (Library.RepaintSerial or 0) + 1
	local serial = Library.RepaintSerial
	for _, key in keys do
		local list = Library.PaintersByKey[key]
		if list then
			for index = #list, 1, -1 do
				local painter = list[index]
				if painter.RepaintSerial == serial then
					continue
				end
				painter.RepaintSerial = serial
...
function Utils.Surface(properties, topKey, bottomKey)
	...
	Utils.BindGradient(gradient, topKey, bottomKey)
...
function Utils.HugWidth(holder, label, padding, minimum)
	...
	local function apply()
		local width = math.max(minimum, math.ceil(label.TextBounds.X) + padding)
		local size = holder.Size
		if size.X.Scale ~= 0 or size.X.Offset ~= width then
			holder.Size = UDim2.new(0, width, size.Y.Scale, size.Y.Offset)
		end
	end
...
function Utils.PointerDrag(target, onMove, onStart, onFinished)
	local activeInput, moveConnection, endConnection
	...
	local function stop()
		...
		activeInput = nil
		Library:Disconnect(moveConnection)
		Library:Disconnect(endConnection)
	...
	Library:Connect(target.InputBegan, function(input)
		...
		activeInput = input
		if onStart then
			onStart(input.Position, input)
		end
		if onMove then
			onMove(input.Position, input)
		end

		moveConnection = Library:Connect(UserInputService.InputChanged, function(changed)
			if not activeInput then
				return
			end
			if activeInput.UserInputType == Enum.UserInputType.Touch then
				if changed.UserInputType == Enum.UserInputType.Touch and onMove then
					onMove(changed.Position, changed)
				end
			elseif changed.UserInputType == Enum.UserInputType.MouseMovement and onMove then
				onMove(changed.Position, changed)
			end
		end)

		endConnection = Library:Connect(UserInputService.InputEnded, function(ended)
			if not activeInput then
				return
			end
			if activeInput.UserInputType == Enum.UserInputType.Touch then
				if ended == activeInput then
					stop()
				end
...
function Utils.Drag(handle, target, onFinished, minY)
	...
	return Utils.PointerDrag(handle, function(position)
		...
		local x = math.floor(Utils.Clamp(startPosition.X.Offset + delta.X, 0, math.max(0, viewport.X - size.X)) + 0.5)
		local y = math.floor(Utils.Clamp(startPosition.Y.Offset + delta.Y, top, math.max(top, viewport.Y - size.Y)) + 0.5)
		local nextPosition = UDim2.fromOffset(x, y)
...
function Library:Window(options)
	...
	if Field(options, "Resizable", true) then
		...
		Utils.PointerDrag(grip, function(position)
			if not startInput or not startSize then
				return
			end
			local delta = position - startInput
			local width = math.max(minSize.X, math.floor(startSize.X + delta.X + 0.5))
			local height = math.max(minSize.Y, math.floor(startSize.Y + delta.Y + 0.5))
			local nextSize = UDim2.fromOffset(width, height)
			if frame.Size ~= nextSize then
				frame.Size = nextSize
...
local function StartPickerLoop()
	if Library.PickerLoop or #Library.AnimatedPickers == 0 then
		return
	end

	local accumulator = 0
	local connection
	connection = Library:Connect(RunService.RenderStepped, function(delta)
		if Library.Unloaded or #Library.AnimatedPickers == 0 then
			Library.PickerLoop = false
		...
		for index = #Library.AnimatedPickers, 1, -1 do
			local picker = Library.AnimatedPickers[index]
			local mode = picker and picker.Animation
			if not picker or not mode or mode == "Off" then
				table.remove(Library.AnimatedPickers, index)
				continue
			end

			local clock = now * (picker.AnimationSpeed or 1)
			local base = typeof(picker.AnimationBase) == "Color3" and picker.AnimationBase or picker.Value
			...
			elseif mode == "Breathe" then
				...
				color = Color3.fromHSV((hue + Utils.Lerp(-0.08, 0.08, alpha)) % 1, Utils.Lerp(saturation * 0.4, saturation, alpha), value)
			elseif mode == "Pulse" then
				color = math.floor(clock * 2) % 2 == 0 and base or Utils.Multiply(base, 0.35)
			elseif mode == "Lerp" then
				local other = typeof(picker.ColourB) == "Color3" and picker.ColourB or Utils.Multiply(base, 0.4)
				color = base:Lerp(other, (math.sin(clock * 2) + 1) / 2)
...
function Library:GetAvatar(userId, callback)
	...
	local pending = Library.AvatarPending[userId]
	if pending then
		if callback then
	...
	pending = {}
	Library.AvatarPending[userId] = pending
	if callback then
		table.insert(pending, callback)
	end
	...
	task.spawn(function()
		...
		local callbacks = Library.AvatarPending[userId] or pending
		Library.AvatarPending[userId] = nil
		if ok and url then
			Library.AvatarCache[userId] = url
			for _, queued in callbacks do
				pcall(queued, url)
			end
		end
	end)
...
function Tab:PlayerList(options)
	...
	Library:Connect(search:GetPropertyChangedSignal("Text"), function()
		...
		task.defer(function()
			...
			if not Library.Unloaded and frame.Parent then
				list:Refresh()
			end
		end)
	end)
...
function Tab:Section(options)
	...
	if hasHeader then
		...
		Utils.BindGradient(headerGradient, "SectionHeaderTop", "SectionHeaderBottom")
...
function Container:Toggle(options)
	...
	Utils.BindGradient(fillGradient, "Accent", "AccentToggleShade")
...
function BuildButton(parent, options, width)
	...
	Utils.BindGradient(highlightGradient, "Accent", "AccentToggleShade")
...
function Container:Slider(options)
	...
	local hitbox = Utils.Hitbox(row, 5)
	Utils.PointerDrag(hitbox, updateFromInput)
...
function Utils.Popup(anchor, width, height, options)
	...
	local function reposition()
		...
		if options.MatchAnchorWidth and size.X > 0 then
			local nextSize = UDim2.fromOffset(math.floor(size.X + 0.5), frame.Size.Y.Offset)
			if frame.Size ~= nextSize then
				frame.Size = nextSize
			end
		end
		...
		local nextPosition = UDim2.fromOffset(math.floor(x), math.floor(y))
		if frame.Position ~= nextPosition then
	...
	function popup:Close()
		...
		if popup.Tracker then
			Library:Disconnect(popup.Tracker)
			popup.Tracker = nil
		end
	...
	if title then
		...
		Utils.BindGradient(stripGradient, "SectionHeaderTop", "SectionHeaderBottom")
...
function Container:Dropdown(options)
	...
	Utils.BindGradient(listGradient, "SectionTop", "SectionBottom")
	...
	local function rebuild()
		...
		for index, item in element.Items do
			...
			Utils.TrackText(button, Metrics.TextBase)
...
local function BuildColorpicker(holder, options, ownerRow)
	...
	local function bindDrag(target, handler)
		Utils.PointerDrag(target, handler)
	end
	...
	animContainer:Dropdown({
		...
		Callback = function(mode)
			...
			if mode == "Off" then
				...
				if #Library.AnimatedPickers == 0 and Library.PickerLoop then
					Library:Disconnect(Library.PickerLoop)
					Library.PickerLoop = false
				end
...
function Library:Unload()
	...
	table.clear(Library.AnimatedPickers)
	table.clear(Library.AvatarCache)
	table.clear(Library.AvatarPending)
	table.clear(Library.ActiveTweens)
	table.clear(Library.Painters)
	table.clear(Library.PaintersByKey)
	table.clear(Library.PainterIndex)
	table.clear(Library.SwatchGradients)
	Library.PickerLoop = false
...
function Library:SetOpen(open, immediate)
	...
	if Library.MenuFade then
		Library:Disconnect(Library.MenuFade)
		Library.MenuFade = false
	end
...
--[[
	Drives Accent over time. The animated colour updates the two accent theme keys
	directly and repaints only their dependent properties; AnimationBase keeps the
	user-selected colour so stopping the animation restores it cleanly.

	The loop is capped at 30Hz, which keeps the motion smooth without repainting
	all accent-dependent properties on every rendered frame.
]]
...
function Library:SetAccentAnimation(mode, speed)
	...
	if mode == "Off" then
		...
		if Library.AnimationThread then
			Library:Disconnect(Library.AnimationThread)
			Library.AnimationThread = false
		end
		return Library
	end
	...
	connection = Library:Connect(RunService.RenderStepped, function(delta)
		...
		local base = typeof(Library.AnimationBase) == "Color3" and Library.AnimationBase or Themes.Default.Accent
		local hue, saturation, value = base:ToHSV()
		local color
		...
		elseif current == "Breathe" then
			local alpha = (math.sin(clock * 1.2) + 1) / 2
			color = Color3.fromHSV((hue + Utils.Lerp(-0.06, 0.06, alpha)) % 1, Utils.Lerp(saturation * 0.5, saturation, alpha), value)
		end
		if not color then
			return
		end
	...
	Library.AnimationThread = connection
	return Library
end
...
function Library:Watermark(options)
	...
	local cachedPing = "N/A"
	local cachedDay = os.date("%d")
	local cachedMonth = os.date("%b"):lower()
	local cachedHour = os.date("%H")
	local cachedMinute = os.date("%M")

	local function sampleDefaultFields()
		cachedDay = os.date("%d")
		cachedMonth = os.date("%b"):lower()
		cachedHour = os.date("%H")
		cachedMinute = os.date("%M")
		local okPing, stat = pcall(function()
			return math.floor(Stats.Network.ServerStatsItem["Data Ping"]:GetValue())
		end)
		cachedPing = okPing and stat or "N/A"
	end

	local function defaultFields()
		return {
			Library.Name,
			accent(cachedDay) .. " " .. accent(cachedMonth),
			accent(cachedHour) .. ":" .. accent(cachedMinute),
			accent(fpsSample) .. " Fps",
			accent(cachedPing) .. " Ping",
		}
	end
...
function Library:PlayerPanel(options)
	...
	Utils.BindGradient(healthGradient, "Accent", "AccentToggleShade")
	...
	for index, entry in lines do
		...
		if label then
			...
			rowPainters[index] = Library:Paint(label, "Text", function()
				return string.format("%s: %s", tostring(entry.Label), accent(rowValues[index]))
			end, { "Accent" })
		end
	end
	...
	local healthPainter = Library:Paint(healthText, "Text", function()
		return panel.HealthValue
	end)

	function panel:Refresh()
		for index, entry in lines do
			local ok, value = pcall(entry.Get)
			rowValues[index] = ok and tostring(value) or "N/A"
			local painter = rowPainters[index]
			if painter then
				painter.Apply()
			end
		end
		...
		if humanoid and humanoid.MaxHealth > 0 then
			alpha = Utils.Clamp(humanoid.Health / humanoid.MaxHealth, 0, 1)
		end
		local nextSize = UDim2.fromScale(alpha, 1)
		if healthFill.Size ~= nextSize then
			healthFill.Size = nextSize
		end
		panel.HealthValue = string.format("%d%%", math.floor(alpha * 100 + 0.5))
		healthPainter.Apply()
	end
...
function Library:LuaWindow(options)
	...
	local highlightScheduled = false
	local function scheduleHighlight()
		...
		task.delay(0.05, function()
			...
			if Library.Unloaded or not input.Parent then
				return
			end
