...
local Configuration = {
    ...
    Steer_Factor = 1.000,
    Anti_Pull = true,
    Pull_Grace_Window = 0.18,
    Pull_Threshold_Boost = 0.12,
...
local Player_State = {
    ...
    Pull_Detected_Time = -999,
    Pull_Last_Velocity = V3_Zero,
}
local Parry_State = {
    Ball = {
        Position = V3_Zero,
...
local function Get_Optimal_Parry_Threshold(Ball_Instance, Ball_Speed, Player_Position, Current_Ping, Distance_To_Ball, Delta_Time)
    ...
    if Configuration.Dot_Protect then
        ...
        if Dot_Product > 0 and Dot_Product <= (Configuration.Dot_Protect_Side_Threshold or 0.65) then
            ...
            if Capped_Speed >= Side_Min_Speed and Distance_To_Ball <= Side_Max_Dist then
                local Side_Angle_Factor = 1 - (Dot_Product / (Configuration.Dot_Protect_Side_Threshold or 0.65))
                local Dist_Factor = 1 - Math_Clamp(Distance_To_Ball / Side_Max_Dist, 0, 1)
                local Side_Limit = Scaled_Ping + Distance_To_Ball * 0.6 + (Side_Angle_Factor * 12) + (Dist_Factor * 8)
                local Side_Floor = Math_Max(Configuration.Dot_Limit_Threshold * 0.85, 45)
                Parry_Threshold = Math_Min(Parry_Threshold, Math_Max(Side_Limit, Side_Floor))
            end
        end
    end
    ...
    if Configuration.Anti_Pull and Time_Since_Pull < Pull_Grace then
        ...
        local Pull_Boost = Capped_Speed * (Configuration.Pull_Threshold_Boost or 0.12) * Pull_Fade
        Parry_Threshold = Parry_Threshold + Pull_Boost
    end
...
Interface_Manager = {
    ...
    Dimensions = V2_New(570, 440),
    Minimum_Dimensions = V2_New(440, 340),
    Is_Dragging = false,
    ...
    Resize_Corners = {},
    Themes = {
        {"Nightfall", {Bg="#0F0F14", Out="#000000", In_Box="#1A1A22", Acc="#A75CFF", Side="#121218", Grp="#14141C", Prim="#FFFFFF", Sec="#A0A4B8", Tog="#1C1C26", Hov="#282836"}},
        {"Bloodmoon", {Bg="#140606", Out="#000000", In_Box="#220E0E", Acc="#FF3333", Side="#180A0A", Grp="#1C0D0D", Prim="#FFFFFF", Sec="#A88282", Tog="#221212", Hov="#321A1A"}},
        {"Ocean", {Bg="#060A14", Out="#000000", In_Box="#0E1622", Acc="#33A7FF", Side="#0A1018", Grp="#0D131C", Prim="#FFFFFF", Sec="#8295A8", Tog="#121A22", Hov="#1A2432"}},
        {"Mint", {Bg="#06140A", Out="#000000", In_Box="#0E2216", Acc="#33FF99", Side="#0A1810", Grp="#0D1C13", Prim="#FFFFFF", Sec="#82A895", Tog="#12221A", Hov="#1A3224"}}
    },
    Palette = {
        Background = C3_Hex("#0F0F14"), Outline = C3_Hex("#000000"), Inline = C3_Hex("#1A1A22"),
        Accent_Color = C3_Hex("#A75CFF"), Sidebar_Background = C3_Hex("#121218"), Group_Background = C3_Hex("#14141C"),
        Primary_Text = C3_Hex("#FFFFFF"), Secondary_Text = C3_Hex("#A0A4B8"), Toggle_Background = C3_Hex("#1C1C26"),
        Hover_State = C3_Hex("#282836")
    }
}
...
local function Execute_Parry_Action(Is_Lobby_Parry_Call)
    ...
    if not Is_Lobby_Parry_Call and Configuration.Auto_Curve then
        local Curve_Mode = Configuration.Auto_Curve_Mode
        local Sensitivity_Val = Configuration.Camera_Sens
        if Curve_Mode == 1 then
            local Delta_Y = -(600 * Sensitivity_Val)
            Did_Curve = true
            Task_Spawn(function()
                if mousemoverel then mousemoverel(0, Delta_Y) end
                Task_Spawn(Do_Click)
                Task_Wait(0.01)
                if mousemoverel then mousemoverel(0, -Delta_Y) end
            end)
        elseif Curve_Mode == 2 then
            local Delta_X = math.floor(5000 * Sensitivity_Val)
            Did_Curve = true
            Task_Spawn(function()
                if mousemoverel then mousemoverel(Delta_X, 0) end
                Task_Wait(0.05)
                Task_Spawn(Do_Click)
                Task_Wait(0.05)
                if mousemoverel then mousemoverel(-Delta_X, 0) end
...
local function Setup_Pull_Detector()
    Safe_Call(function()
        local Runtime_Folder = Workspace_Service:FindFirstChild("Runtime")
        if not Runtime_Folder then return end
        if Pull_Detector.Runtime_Conn then
            Pull_Detector.Runtime_Conn:Disconnect()
            Pull_Detector.Runtime_Conn = nil
        end
        Pull_Detector.Runtime_Conn = Cvm:OnChildAdded(Runtime_Folder, function(Child_Instance)
            local Child_Name = Child_Instance.Name
...
local function Run_Loader()
    ...
    local BG = ND("Square", {Filled=true, Visible=true, Color=Color3.fromHex("#0F0F14"), Transparency=0, Position=Vector2.new(0,0), Size=Screen_Size, ZIndex=1})
    local P_Outline = ND("Square", {Filled=true, Visible=true, Color=Color3.fromHex("#000000"), Transparency=0, Position=Vector2.new(Panel_X-1,Panel_Y-1), Size=Vector2.new(Panel_W+2,Panel_H+2), Rounding=4, ZIndex=3})
    local P_BG = ND("Square", {Filled=true, Visible=true, Color=Color3.fromHex("#0F0F14"), Transparency=0, Position=Vector2.new(Panel_X,Panel_Y), Size=Vector2.new(Panel_W,Panel_H), Rounding=4, ZIndex=4})
    local P_Inner = ND("Square", {Filled=true, Visible=true, Color=Color3.fromHex("#14141C"), Transparency=0, Position=Vector2.new(Panel_X+1,Panel_Y+1), Size=Vector2.new(Panel_W-2,Panel_H-2), Rounding=4, ZIndex=4})
    ...
    local Title = ND("Text", {Text="NIGHTFALL", Font=2, Size=24, Color=Color3.fromHex("#FFFFFF"), Outline=false, Center=true, Visible=true, Transparency=0, Position=Vector2.new(Cx, Panel_Y+22), ZIndex=6})
    local Sub = ND("Text", {Text="initializing", Font=2, Size=12, Color=Color3.fromHex("#A0A4B8"), Outline=false, Center=true, Visible=true, Transparency=0, Position=Vector2.new(Cx, Panel_Y+52), ZIndex=6})
    local Exp = ND("Text", {Text="Expires: Never", Font=2, Size=11, Color=Color3.fromHex("#A0A4B8"), Outline=false, Center=true, Visible=true, Transparency=0, Position=Vector2.new(Cx, Panel_Y+72), ZIndex=6})
    local Bar_W = Panel_W - 60
    local Bar_X = Panel_X + 30
    local Bar_Y = Panel_Y + Panel_H - 52
    local Bar_H = 3
    local Bar_Track = ND("Square", {Filled=true, Visible=true, Color=Color3.fromHex("#000000"), Transparency=0, Position=Vector2.new(Bar_X-1,Bar_Y-1), Size=Vector2.new(Bar_W+2,Bar_H+2), Rounding=0, ZIndex=6})
    local Bar_BG = ND("Square", {Filled=true, Visible=true, Color=Color3.fromHex("#1C1C26"), Transparency=0, Position=Vector2.new(Bar_X,Bar_Y), Size=Vector2.new(Bar_W,Bar_H), Rounding=0, ZIndex=7})
    local Bar_Fill = ND("Square", {Filled=true, Visible=true, Color=Color3.fromHex("#A75CFF"), Transparency=0, Position=Vector2.new(Bar_X,Bar_Y), Size=Vector2.new(0,Bar_H), Rounding=0, ZIndex=8})
    local Pct = ND("Text", {Text="0%", Font=2, Size=11, Color=Color3.fromHex("#A0A4B8"), Outline=false, Center=true, Visible=true, Transparency=0, Position=Vector2.new(Cx, Bar_Y+10), ZIndex=6})
    local Arc_R = 20
    local Arc_Cx = Cx
    ...
    local Dots = {}
    for i = 1, Dot_N do
        Dots[i] = ND("Circle", {Filled=true, Visible=true, Color=Color3.fromHex("#D4AAFF"), Transparency=0, Radius=2.5, Position=Vector2.new(Arc_Cx,Arc_Cy), ZIndex=9})
    end
    local function All_Objs()
        return {BG, P_Outline, P_BG, P_Inner, Acc_Line, Title, Sub, Exp, Bar_Track, Bar_BG, Bar_Fill, Pct}
    end
...
local function Construct_User_Interface()
    Interface_Manager.Main_Outline = Instantiate_Drawing("Square", {Filled = true, Visible = true, Transparency = 1, Rounding = 4}, "Outline")
    Interface_Manager.Main_Background = Instantiate_Drawing("Square", {Filled = true, Visible = true, Transparency = 1, Rounding = 4}, "Background")
    Interface_Manager.Sidebar_Area = Instantiate_Drawing("Square", {Filled = true, Visible = true, Transparency = 1, Rounding = 4}, "Sidebar_Background")
    Interface_Manager.Sidebar_Divider = Instantiate_Drawing("Square", {Filled = true, Visible = true, Transparency = 1}, "Outline")
    ...
    Interface_Manager.Title_Text = Instantiate_Drawing("Text", {Text = "NIGHTFALL", Size = 16, Font = 2, Outline = true, Visible = true, Transparency = 1}, "Primary_Text")
    Interface_Manager.Tab_Renderers = {}
    for Index_Val, Tab_Name in ipairs(Interface_Manager.Navigation_Tabs) do
        local Text_Renderer = Instantiate_Drawing("Text", {Text = Tab_Name, Size = 14, Font = 2, Outline = true, Visible = true, Transparency = 1})
    ...
    local function Create_Group_Container(Tab_Name, Section_Name, Grid_Side)
        local Container_Data = {
            Outline_Box = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Outline"),
            Background_Box = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Group_Background"),
            Title_Background = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Background"),
            Title_Text = Instantiate_Drawing("Text", {Text = Section_Name, Size = 13, Font = 2, Outline = true, Visible = false, Transparency = 1}, "Primary_Text"),
    ...
    local function Create_Toggle_Element(Parent_Group, Display_Name, Config_Identifier, Keybind_Identifier)
        local Vertical_Offset = Parent_Group.Item_Count * 28
        ...
        local Toggle_Element = {
            Outline_Box = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Outline, Filled = true, Visible = false, Transparency = 1, Rounding = 4}),
            Inline_Box = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Inline, Filled = true, Visible = false, Transparency = 1, Rounding = 4}),
            Fill_Box = Instantiate_Drawing("Square", {Color = Default_Fill_Color, Filled = true, Visible = false, Transparency = 1, Rounding = 0}),
            Label_Text = Instantiate_Drawing("Text", {Text = Display_Name, Color = Default_Text_Color, Size = 13, Font = 2, Outline = true, Visible = false, Transparency = 1}),
        ...
        if Keybind_Identifier then
            local Keybind_Element = {
                Outline_Box = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Outline, Filled = true, Visible = false, Transparency = 1, Rounding = 4}),
                Inline_Box = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Toggle_Background, Filled = true, Visible = false, Transparency = 1, Rounding = 4}),
                Label_Text = Instantiate_Drawing("Text", {Text = "[ - ]", Color = Interface_Manager.Palette.Secondary_Text, Size = 12, Font = 2, Outline = true, Center = true, Visible = false, Transparency = 1}),
    ...
    local function Create_Dropdown_Element(Parent_Group, Display_Name, Config_Identifier, Option_List, Is_Theme_Dropdown)
        ...
        local Dropdown_Element = {
            Outline_Box = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Outline, Filled = true, Visible = false, Transparency = 1, Rounding = 4}),
            Inline_Box = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Toggle_Background, Filled = true, Visible = false, Transparency = 1, Rounding = 4}),
            Label_Text = Instantiate_Drawing("Text", {Text = String_Format("%s: %s", Display_Name, Option_List[Configuration[Config_Identifier]]), Color = Interface_Manager.Palette.Secondary_Text, Size = 13, Font = 2, Outline = true, Center = false, Visible = false, Transparency = 1}),
            State_Icon = Instantiate_Drawing("Text", {Text = "+", Color = Interface_Manager.Palette.Secondary_Text, Size = 13, Font = 2, Outline = true, Center = false, Visible = false, Transparency = 1}),
    ...
    local function Create_Slider_Element(Parent_Group, Display_Name, Config_Identifier, Min_Value, Max_Value, Decimal_Places, Suffix_Text)
        ...
        local Slider_Element = {
            Label_Text = Instantiate_Drawing("Text", {Text = Display_Name, Size = 13, Font = 2, Outline = true, Visible = false, Transparency = 1}, "Secondary_Text"),
            Value_Text = Instantiate_Drawing("Text", {Text = "", Size = 13, Font = 2, Outline = true, Visible = false, Transparency = 1}, "Primary_Text"),
            Outline_Box = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Outline"),
            Background_Box = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Toggle_Background"),
            Fill_Indicator = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Accent_Color"),
    ...
    local function Create_Color_Picker(Parent_Group, Color_Key_Target)
        local Vertical_Offset = Parent_Group.Item_Count * 28
        local Cp_Data = {
            Sv_Outline = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 2}, "Outline"),
            Sv_Background = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 2}),
            Sv_Blocks = {},
            Hue_Outline = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 2}, "Outline"),
            Hue_Blocks = {},
            Alpha_Outline = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 2}, "Outline"),
            Alpha_Blocks = {},
            Details_Text1 = Instantiate_Drawing("Text", {Text = "", Size = 13, Font = 2, Outline = true, Visible = false, Transparency = 1}, "Primary_Text"),
            Details_Text2 = Instantiate_Drawing("Text", {Text = "", Size = 13, Font = 2, Outline = true, Visible = false, Transparency = 1}, "Primary_Text"),
            Hex_Text = Instantiate_Drawing("Text", {Text = "", Size = 13, Font = 2, Outline = true, Visible = false, Transparency = 1}, "Accent_Color"),
        ...
        Parent_Group.Item_Count = Parent_Group.Item_Count + 6.5
    end
    local function Create_Full_Keybind_Element(Parent_Group, Display_Name, Config_Identifier)
        ...
        local Keybind_Element = {
            ...
            Outline_Box = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Outline, Filled = true, Visible = false, Transparency = 1, Rounding = 4}),
            Inline_Box = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Toggle_Background, Filled = true, Visible = false, Transparency = 1, Rounding = 4}),
            Label_Text = Instantiate_Drawing("Text", {Text = Formatted_Text, Color = Interface_Manager.Palette.Secondary_Text, Size = 13, Font = 2, Outline = true, Center = true, Visible = false, Transparency = 1}),
    ...
    local function Create_Button(Parent_Group, Display_Name, Callback_Func)
        ...
        local Btn_Element = {
            Outline_Box = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Outline, Filled = true, Visible = false, Transparency = 1, Rounding = 4}),
            Inline_Box = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Toggle_Background, Filled = true, Visible = false, Transparency = 1, Rounding = 4}),
            Label_Text = Instantiate_Drawing("Text", {Text = Display_Name, Color = Interface_Manager.Palette.Primary_Text, Size = 13, Font = 2, Outline = true, Center = true, Visible = false, Transparency = 1}),
    ...
    Create_Toggle_Element(Combat_Main_Section, "Anti Pull", "Anti_Pull", nil)
    ...
    Interface_Manager.Overlay_Indicator = {
        Outline_Box = Instantiate_Drawing("Square", {Filled = true, Visible = true, Transparency = 1, Rounding = 4}, "Outline"),
        Inline_Box = Instantiate_Drawing("Square", {Filled = true, Visible = true, Transparency = 1, Rounding = 4}, "Inline"),
        Background_Box = Instantiate_Drawing("Square", {Filled = true, Visible = true, Transparency = 1, Rounding = 4}, "Group_Background"),
        Accent_Line = Instantiate_Drawing("Square", {Filled = true, Visible = true, Transparency = 1}, "Accent_Color"),
        Title_Text = Instantiate_Drawing("Text", {Text = "keybinds", Size = 13, Font = 2, Outline = true, Visible = true, Transparency = 1}, "Primary_Text"),
        Render_Rows = {}
    }
    for _ = 1, 5 do
        Table_Insert(Interface_Manager.Overlay_Indicator.Render_Rows, {
            Name_Text = Instantiate_Drawing("Text", {Text = "", Size = 13, Font = 2, Outline = true, Visible = true, Transparency = 1}, "Primary_Text"),
            State_Text = Instantiate_Drawing("Text", {Text = "", Size = 13, Font = 2, Outline = true, Visible = true, Transparency = 1})
        })
    end
    Interface_Manager.Stats_Panel = {
        Outline_Box = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Outline"),
        Inline_Box = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Inline"),
        Background_Box = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Group_Background"),
        Accent_Line = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1}, "Accent_Color"),
        Title_Text = Instantiate_Drawing("Text", {Text = "GENERAL", Size = 12, Font = 2, Outline = true, Visible = false, Transparency = 1}, "Secondary_Text"),
        Render_Rows = {}
    }
    ...
    for Index_Val = 1, 3 do
        Table_Insert(Interface_Manager.Stats_Panel.Render_Rows, {
            Label_Text = Instantiate_Drawing("Text", {Text = Stat_Identifiers[Index_Val], Size = 13, Font = 2, Outline = true, Visible = false, Transparency = 1}, "Primary_Text"),
            Value_Text = Instantiate_Drawing("Text", {Text = "-", Size = 13, Font = 2, Outline = true, Visible = false, Transparency = 1}, "Accent_Color"),
            Divider_Line = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1}, "Outline")
        })
    end
    Interface_Manager.Manual_Spam_Panel = {
        ...
        Background_Box = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Group_Background"),
        Accent_Line = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1}, "Accent_Color"),
        Title_Text = Instantiate_Drawing("Text", {Text = "MANUAL SPAM", Size = 13, Font = 2, Outline = true, Visible = false, Transparency = 1}, "Primary_Text"),
        State_Text = Instantiate_Drawing("Text", {Text = "OFF", Size = 16, Font = 2, Outline = true, Visible = false, Transparency = 1, Center = true}, "Secondary_Text"),
    }
    Interface_Manager.Context_Menu_Panel = {
        Outline_Box = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Outline"),
        Background_Box = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 4}, "Background"),
        Accent_Line = Instantiate_Drawing("Square", {Filled = true, Visible = false, Transparency = 1, Rounding = 0}, "Accent_Color"),
        Title_Text = Instantiate_Drawing("Text", {Text = "BIND MODE", Size = 11, Font = 2, Outline = false, Visible = false, Transparency = 1}, "Secondary_Text"),
        Divider = Instantiate_Drawing("Line", {Thickness = 1, Visible = false, Transparency = 1}, "Outline"),
        ...
        Option1_Text = Instantiate_Drawing("Text", {Text = "Toggle", Size = 13, Font = 2, Outline = false, Visible = false, Transparency = 1}, "Primary_Text"),
        ...
        Option2_Text = Instantiate_Drawing("Text", {Text = "Hold", Size = 13, Font = 2, Outline = false, Visible = false, Transparency = 1}, "Primary_Text"),
    }
    for _, Dropdown_Data in ipairs(Uninitialized_Dropdowns) do
        Dropdown_Data.List_Outline = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Outline, Filled = true, Transparency = 1, Rounding = 4, Visible = false})
        Dropdown_Data.List_Inline = Instantiate_Drawing("Square", {Color = Interface_Manager.Palette.Group_Background, Filled = true, Transparency = 1, Rounding = 4, Visible = false})
        for _, Option_String in ipairs(Dropdown_Data.Option_Data) do
            Table_Insert(Dropdown_Data.Item_Renderers, {
                Label_Text = Instantiate_Drawing("Text", {Text = Option_String, Color = Interface_Manager.Palette.Secondary_Text, Size = 13, Font = 2, Outline = true, Transparency = 0, Visible = false}),
...
local function Refresh_Layout_Coordinates()
    ...
    for _, Tab_Data in pairs(Interface_Manager.Tab_Renderers) do
        local Layout_Y = 55 + (Tab_Data.Render_Index * 32)
    ...
    for _, Group_Data in ipairs(Interface_Manager.Group_Container_List) do
        ...
        local Layout_Y = Base_Pos.Y + 25
        Group_Data.Outline_Box.Position = V2_New(Layout_X, Layout_Y)
        Group_Data.Outline_Box.Size = V2_New(Calculated_Group_Width, Base_Size.Y - 45)
        Group_Data.Background_Box.Position = V2_New(Layout_X + 1, Layout_Y + 1)
        Group_Data.Background_Box.Size = V2_New(Calculated_Group_Width - 2, Base_Size.Y - 47)
    ...
    for _, Dropdown_Data in ipairs(Interface_Manager.Dropdowns) do
        ...
        Dropdown_Data.Outline_Box.Size = V2_New(Layout_Width, 24)
        Dropdown_Data.Inline_Box.Position = V2_New(Layout_X + 1, Layout_Y + 1)
        Dropdown_Data.Inline_Box.Size = V2_New(Layout_Width - 2, 22)
        ...
        Dropdown_Data.Label_Text.Position = V2_New(Layout_X + 8, Layout_Y + 6)
        Dropdown_Data.State_Icon.Position = V2_New(Layout_X + Layout_Width - 16, Layout_Y + 6)
        Dropdown_Data.List_Outline.Position = V2_New(Layout_X, Layout_Y + 25)
        Dropdown_Data.List_Outline.Size = V2_New(Layout_Width, Dropdown_Data.Current_Height)
        Dropdown_Data.List_Inline.Position = V2_New(Layout_X + 1, Layout_Y + 26)
        ...
        for Option_Index, Option_Renderer in ipairs(Dropdown_Data.Item_Renderers) do
            local Option_Layout_Y = Layout_Y + 26 + ((Option_Index - 1) * 20)
    ...
    for _, Bind_Data in ipairs(Interface_Manager.Binds) do
        ...
        if Bind_Data.Is_Full_Width then
            ...
            Bind_Data.Outline_Box.Size = V2_New(Layout_Width, 24)
            Bind_Data.Inline_Box.Position = V2_New(Layout_X + 1, Layout_Y + 1)
            Bind_Data.Inline_Box.Size = V2_New(Layout_Width - 2, 22)
            Bind_Data.Label_Text.Position = V2_New(Layout_X + (Layout_Width / 2), Layout_Y + 13)
        else
            Bind_Data.Outline_Box.Position = V2_New(Layout_X + Layout_Width - 45, Layout_Y - 3)
            Bind_Data.Outline_Box.Size = V2_New(45, 18)
            Bind_Data.Inline_Box.Position = V2_New(Layout_X + Layout_Width - 44, Layout_Y - 2)
            Bind_Data.Inline_Box.Size = V2_New(43, 16)
            Bind_Data.Label_Text.Position = V2_New(Layout_X + Layout_Width - 22.5, Layout_Y + 6)
        end
    end
    for _, Btn_Data in ipairs(Interface_Manager.Buttons) do
        ...
        Btn_Data.Outline_Box.Size = V2_New(Layout_Width, 24)
        Btn_Data.Inline_Box.Position = V2_New(Layout_X + 1, Layout_Y + 1)
        Btn_Data.Inline_Box.Size = V2_New(Layout_Width - 2, 22)
        Btn_Data.Label_Text.Position = V2_New(Layout_X + (Layout_Width / 2), Layout_Y + 12)
    end
...
Task_Spawn(function()
    ...
    while _G.Nightfall_Active do
        ...
        if Should_Render_Keybinds then
            ...
            Interface_Manager.Overlay_Indicator.Render_Rows[5].Name_Text.Text = String_Format("[%s] Manual", Format_Keycode_Name(Configuration.Manual_Spam_Keybind))
            Interface_Manager.Overlay_Indicator.Render_Rows[5].State_Text.Text = Player_State.Manual_Spam_Active and "[ON]" or "[OFF]"
            Interface_Manager.Overlay_Indicator.Render_Rows[5].State_Text.Color = Player_State.Manual_Spam_Active and Interface_Manager.Palette.Accent_Color or Interface_Manager.Palette.Secondary_Text
        end
            ...
            if Interface_Manager.Is_Visible and not Interface_Manager.Is_Resizing and not Interface_Manager.Is_Dragging and not Interface_Manager.Is_Dragging_Indicator and not Interface_Manager.Is_Dragging_Stats and not Interface_Manager.Is_Dragging_Manual_Spam then
                if Mouse_Position.X >= Interface_Manager.Base_Position.X and Mouse_Position.X <= Interface_Manager.Base_Position.X + 130 then
                    for Tab_Name, Renderer_Data in pairs(Interface_Manager.Tab_Renderers) do
                        if Mouse_Position.Y >= Interface_Manager.Base_Position.Y + 45 + (Renderer_Data.Render_Index * 32) and Mouse_Position.Y <= Interface_Manager.Base_Position.Y + 65 + (Renderer_Data.Render_Index * 32) then
        ...
        while _G.Nightfall_Active do
            if Player_State.Manual_Spam_Active and isrbxactive() then
                ...
                if Player_State.Manual_Spam_Active and isrbxactive() then
                    ...
                    local Calc_Delay = 0.20 - ((Configuration.Manual_Spam_Speed - 10) / 90) * 0.17
                    local Speed_Tick = tick()
                    while tick() - Speed_Tick < Calc_Delay do
                        if not Player_State.Manual_Spam_Active then break end
                        Task_Wait()
                    end
                end
...
Custom_Run_Service.Heartbeat:Connect(function(Delta_Time)
    ...
    if Ball_Folder_Check then
        for _, Ball_Instance in ipairs(Ball_Folder_Check:GetChildren()) do
            if Ball_Instance:GetAttribute("realBall") then
                ...
                Best_Global_Ball = Ball_Instance
            end
        end
    end
    if Training_Folder_Check then
        for _, Training_Ball in ipairs(Training_Folder_Check:GetChildren()) do
            if Training_Ball:GetAttribute("realBall") then
                ...
                Ball_Entity_Found = true
                Best_Global_Ball = Training_Ball
            end
        end
    end
    ...
    Safe_Call(function()
        ...
        if Configuration.Render_Ball_Stats and Best_Global_Ball then
            Player_State.Tracked_Speed = Get_Vector_Magnitude(Best_Global_Ball.AssemblyLinearVelocity)
            Player_State.Tracked_Distance = Get_Distance_Between(Player_State.Entity.Server_Position, Best_Global_Ball.Position)
            Player_State.Tracked_Dot_Product = Player_State.Tracked_Dot_Product
        end
        if Is_Entity_Alive and (Configuration.Auto_Parry or Configuration.Auto_Spam or Configuration.Triggerbot_Enabled) then
            ...
            if Ball_Folder then
                for _, Ball_Instance in ipairs(Ball_Folder:GetChildren()) do
                    if Ball_Instance:GetAttribute("realBall") then
                        ...
                        local Prev_Ball_Velocity = Parry_State.Ball.Velocity
                        Parry_State.Ball.Position = Ball_Instance.Position
                        Parry_State.Ball.Velocity = Ball_Instance.AssemblyLinearVelocity
                        local current_velocity_mag = Get_Vector_Magnitude(Parry_State.Ball.Velocity)
                        Parry_State.Ball.Speed = current_velocity_mag
                        ...
                        if Target_Attribute_Name ~= Parry_State.Target.Current_Name then
                            ...
                            if Is_Targeting_Local_Player then
                                local Vel_Dot = Get_Dot_Product(
                                    Normalize_Vector(Flatten_Vector(Prev_Ball_Velocity)),
                                    Normalize_Vector(Flatten_Vector(Parry_State.Ball.Velocity))
                                )
                                if Vel_Dot < -0.2 and Configuration.Anti_Pull then
                                    Pull_Detector.Detected_Time = Application_Tick
                                    Pull_Detector.Active = true
                                end
                            end
