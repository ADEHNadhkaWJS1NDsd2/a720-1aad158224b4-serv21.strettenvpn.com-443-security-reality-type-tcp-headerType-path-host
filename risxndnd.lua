...
local function Check_Is_Spam(Spam_Params)
    ...
    local Base_Range = Scaled_Ping + math.min(Spam_Params.Speed / 6.5, 95.0)
    ...
    local Maximum_Speed = 5.0 - math.min(Spam_Params.Speed / 5.0, 5.0)
    local Maximum_Dot = math.clamp(Spam_Params.Dot, -1, 0) * Maximum_Speed
    local Accuracy_Val = math.min(Range_Val - Maximum_Dot, 30.0) * Config.SpamSensitivity
...
Run_Service.Heartbeat:Connect(function(Delta_Time)
    ...
    local Direction_To_Player = Delta_Vector.Unit
    local Dot_Product = Direction_To_Player:Dot(Velocity_Dir)
    ...
    local _, Distance_To_Nearest_Player = Scan_For_Nearest_Entity(Root_Position)
    ...
    if Config.AutoSpam and Check_Is_Spam(Spam_Params) then
        local Spam_Interval = 1 / Config.SpamRate
    ...
    if Current_Target_Attr == Local_Player.Name or Current_Target_Attr == Local_Player.Character or Current_Target_Attr == Local_Player then
        Is_Target_Me = true
    end
    ...
    if Is_Target_Me then
        ...
        local Final_Threshold = Fast_Max((Current_Speed / Speed_Divisor), Parry_Range) + Frame_Distance_Compensation
        ...
        if Current_Speed > 10 and Current_Distance > Close_Range_Threshold then
            ...
            local Distance_Factor_Curved = Fast_Clamp((Current_Distance - Close_Range_Threshold) / 25, 0, 1)
        ...
        if Distance_To_Line <= Final_Threshold and not Is_Curved and not Is_Moving_Away then
            if Config.AutoParry then
                ...
                Execute_Parry()
                Last_Trigger_Time = 0
            elseif Config.TriggerBot and Is_Alive_Status then
                local Delay_Sec = (tonumber(Config.TriggerDelay) or 0) / 1000
                if Delay_Sec > 0 then
                    if Last_Trigger_Time == 0 then
                        Last_Trigger_Time = Fast_Clock()
                    elseif Fast_Clock() - Last_Trigger_Time >= Delay_Sec then
                        Is_Parried = true
                        Execute_Parry()
                        Last_Trigger_Time = 0
                    end
