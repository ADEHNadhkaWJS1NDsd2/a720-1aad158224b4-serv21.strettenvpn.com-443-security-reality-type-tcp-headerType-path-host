...
if not isfolder(Config_Cool_Snake_Case.ConfigFolder) then makefolder(Config_Cool_Snake_Case.ConfigFolder) end
local Theme = {
...
local Base_Size_Cool_Snake_Case = Is_Mobile_Device_Cool_Snake_Case and UDim2.new(0, 450, 0, 260) or UDim2.new(0, 550, 0, 320)
local Sidebar_Width_Cool_Snake_Case = Is_Mobile_Device_Cool_Snake_Case and 130 or 140
...
local function Tween(obj, props, time, style, dir)
    time = time or Config_Cool_Snake_Case.Duration
...
local function Make_Draggable_Cool_Snake_Case(Drag_Area_Cool_Snake_Case, Target_Frame_Cool_Snake_Case, Click_Callback_Cool_Snake_Case)
    ...
    local c3 = Drag_Area_Cool_Snake_Case.InputChanged:Connect(function(input)
    ...
    local c4 = RunService.RenderStepped:Connect(function()
        if Is_Dragging_Cool_Snake_Case and Drag_Input_Cool_Snake_Case then
            ...
            if Delta_Cool_Snake_Case.Magnitude > Drag_Dead_Zone_Cool_Snake_Case then
                Actually_Dragging_Cool_Snake_Case = true
                local scaleObj = Target_Frame_Cool_Snake_Case:FindFirstChildWhichIsA("UIScale")
                local sMult = scaleObj and scaleObj.Scale or 1
                local Viewport_Size_Cool_Snake_Case = workspace.CurrentCamera.ViewportSize
                local Limit_X_Cool_Snake_Case = (Viewport_Size_Cool_Snake_Case.X / 2 / sMult) - (Target_Frame_Cool_Snake_Case.AbsoluteSize.X / 2 / sMult)
                local Limit_Y_Cool_Snake_Case = (Viewport_Size_Cool_Snake_Case.Y / 2 / sMult) - (Target_Frame_Cool_Snake_Case.AbsoluteSize.Y / 2 / sMult)
                local New_X_Cool_Snake_Case = math.clamp(Start_Pos_Cool_Snake_Case.X.Offset + (Delta_Cool_Snake_Case.X / sMult), -Limit_X_Cool_Snake_Case, Limit_X_Cool_Snake_Case)
                local New_Y_Cool_Snake_Case = math.clamp(Start_Pos_Cool_Snake_Case.Y.Offset + (Delta_Cool_Snake_Case.Y / sMult), -Limit_Y_Cool_Snake_Case, Limit_Y_Cool_Snake_Case)
                Target_Frame_Cool_Snake_Case.Position = UDim2.new(0.5, New_X_Cool_Snake_Case, 0.5, New_Y_Cool_Snake_Case)
            end
        end
    end)
...
local function Make_Resizable_Cool_Snake_Case(Resize_Btn_Cool_Snake_Case, Target_Frame_Cool_Snake_Case, Min_Size_Cool_Snake_Case, Max_Size_Cool_Snake_Case)
    if Is_Mobile_Device_Cool_Snake_Case then return end
    local Is_Dragging_Cool_Snake_Case = false
    local Drag_Input_Cool_Snake_Case, Drag_Start_Cool_Snake_Case, Start_Size_Cool_Snake_Case, Start_Pos_Cool_Snake_Case
    local c1 = Resize_Btn_Cool_Snake_Case.InputBegan:Connect(function(input)
    ...
    local c4 = RunService.RenderStepped:Connect(function()
        if Is_Dragging_Cool_Snake_Case and Drag_Input_Cool_Snake_Case then
            ...
            local scaleObj = Target_Frame_Cool_Snake_Case:FindFirstChildWhichIsA("UIScale")
            local sMult = scaleObj and scaleObj.Scale or 1
            local rawX = Start_Size_Cool_Snake_Case.X.Offset + (Delta_Cool_Snake_Case.X / sMult)
            local rawY = Start_Size_Cool_Snake_Case.Y.Offset + (Delta_Cool_Snake_Case.Y / sMult)
            local New_X_Cool_Snake_Case = math.clamp(rawX, Min_Size_Cool_Snake_Case.X, Max_Size_Cool_Snake_Case.X)
            local New_Y_Cool_Snake_Case = math.clamp(rawY, Min_Size_Cool_Snake_Case.Y, Max_Size_Cool_Snake_Case.Y)
...
local function Setup_Smart_Scale_Cool_Snake_Case(Frame_Cool_Snake_Case, Scale_Obj_Cool_Snake_Case)
    local function Update_Scale()
        local vp = workspace.CurrentCamera.ViewportSize
        if vp.X < 1 or vp.Y < 1 then return end
        local tX = Frame_Cool_Snake_Case.Size.X.Offset
        local tY = Frame_Cool_Snake_Case.Size.Y.Offset
        local sX = vp.X / tX
        local sY = vp.Y / tY
        local minS = math.min(sX, sY)
        if minS < 1.05 then
            Scale_Obj_Cool_Snake_Case.Scale = minS * 0.9
        else
            Scale_Obj_Cool_Snake_Case.Scale = 1
        end
    end
    table.insert(Library.Connections, workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(Update_Scale))
    Update_Scale()
end
function Library:Unload()
    ...
    for _, g in pairs(GetParent():GetChildren()) do
        if g.Name == "PrismaMini" or g.Name == Config_Cool_Snake_Case.Name or g.Name == "PrismaKeybinds" or g.Name == "PrismaLoader" or g.Name == "PhantomNotifications" or g.Name == "PhantomWatermark" or g.Name == "PhantomTooltip" or g.Name == "PhantomMiniButton" then
...
function Library:GetConfigs()
    local configs = {}
    if isfolder(Config_Cool_Snake_Case.ConfigFolder) then
        local files = listfiles(Config_Cool_Snake_Case.ConfigFolder)
...
function Library:SaveConfig(name)
    ...
    if ok then
        pcall(function()
            writefile(Config_Cool_Snake_Case.ConfigFolder .. "/" .. name .. ".json", json)
        end)
        return true
    end
    return false
...
function Library:LoadConfig(name)
    if not name or name == "" then return false end
    local path = Config_Cool_Snake_Case.ConfigFolder .. "/" .. name .. ".json"
...
function Library:ConfigExists(name)
    if not name or name == "" then return false end
    return isfile(Config_Cool_Snake_Case.ConfigFolder .. "/" .. name .. ".json")
end
...
function Library:InitWatermark()
    ...
    conn = RunService.RenderStepped:Connect(function()
        ...
        if now - lastUpdate >= 1 then
            ...
            local text = string.format(" <font color='#%s'>%s</font> | FPS: %d | Ping: %sms | %s ", Theme.Accent:ToHex(), Config_Cool_Snake_Case.Name, fps, ping, timeStr)
            Label.Text = text
...
function Library:CreateKeybindList()
    ...
    Make_Draggable_Cool_Snake_Case(Frame, Frame, nil)
...
local function CreateDropdownElement(text, flag, options, default, tooltipText, callback, parentFrame, sectionRef, isMulti, customParent)
    ...
    local Parent_Page_Cool_Snake_Case = parent:FindFirstAncestorOfClass("ScrollingFrame")
    local function CloseDropdown()
        ...
        if Parent_Page_Cool_Snake_Case then Parent_Page_Cool_Snake_Case.ScrollingEnabled = true end
        local c2
    ...
    local optionBtns = {}
    local Drop_Conns_List_Cool_Snake_Case = {}
    ...
    local function BuildOptions(newOptions)
        for _, conn in ipairs(Drop_Conns_List_Cool_Snake_Case) do conn:Disconnect() end
        table.clear(Drop_Conns_List_Cool_Snake_Case)
        ...
        for _, opt in ipairs(options) do
            ...
            table.insert(Drop_Conns_List_Cool_Snake_Case, c3)
            table.insert(Drop_Conns_List_Cool_Snake_Case, c4)
            table.insert(Drop_Conns_List_Cool_Snake_Case, c5)
        end
    end
    ...
    local c6 = Interactive.MouseButton1Click:Connect(function()
        ...
        if Parent_Page_Cool_Snake_Case then Parent_Page_Cool_Snake_Case.ScrollingEnabled = not isDropped end
        if isDropped then
...
function Library:CreateWindow(options)
    if options and options.Name then Config_Cool_Snake_Case.Name = options.Name end
    if options and options.ConfigFolder then Config_Cool_Snake_Case.ConfigFolder = options.ConfigFolder end
    if not isfolder(Config_Cool_Snake_Case.ConfigFolder) then makefolder(Config_Cool_Snake_Case.ConfigFolder) end
    Library:Unload()
    Library:InitWatermark()
    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = Config_Cool_Snake_Case.Name
    ...
    local function Create_Base_Frame_Cool_Snake_Case(name)
        local Frame = Instance.new("Frame")
        Frame.Name = name
        Frame.Size = Base_Size_Cool_Snake_Case
        ...
        Scale.Scale = 1
        Scale.Parent = Frame
        Setup_Smart_Scale_Cool_Snake_Case(Frame, Scale)
        Create_Drag_Zones_Cool_Snake_Case(Frame)
        return Frame, Scale
    end
    local Main_Window_Cool_Snake_Case, MainScale = Create_Base_Frame_Cool_Snake_Case("MainWindow")
    local Settings_Window_Cool_Snake_Case, SetScale = Create_Base_Frame_Cool_Snake_Case("SettingsWindow")
    Library._MainWindow = Main_Window_Cool_Snake_Case
    Library._MainScale = MainScale
    Library._SettingsWindow = Settings_Window_Cool_Snake_Case
    ...
    Resizer.Parent = Main_Window_Cool_Snake_Case
    Resizer.ZIndex = 20
    Resizer.Active = true
    Resizer.Visible = not Is_Mobile_Device_Cool_Snake_Case
    ...
    table.insert(Library.Connections, c2)
    table.insert(Library.Connections, c3)
    Make_Resizable_Cool_Snake_Case(Resizer, Main_Window_Cool_Snake_Case, Vector2.new(450, 300), Vector2.new(800, 500))
    local function CreateSidebar(parent, isSettings)
        local Bar = Instance.new("Frame")
        Bar.Size = UDim2.new(0, Sidebar_Width_Cool_Snake_Case, 1, 0)
        ...
        if isSettings then
            ...
            List.SortOrder = Enum.SortOrder.LayoutOrder
            List.Parent = Container
            local C_Padding = Instance.new("UIPadding")
            C_Padding.PaddingBottom = UDim.new(0, 20)
            C_Padding.Parent = Container
            return Bar, Container, nil
        end
    end
    local MainBar, TabContainer, _ = CreateSidebar(Main_Window_Cool_Snake_Case, false)
    local SetBar, SetContainer, BackBtn = CreateSidebar(Settings_Window_Cool_Snake_Case, true)
    ...
    SideName.Size = UDim2.new(1, -60, 0, 16)
    SideName.Position = UDim2.new(0, 58, 0.5, -9)
    ...
    SideSub.Size = UDim2.new(1, -60, 0, 14)
    SideSub.Position = UDim2.new(0, 58, 0.5, 9)
    ...
    local function ToggleMain()
        ...
        if Library.Open then
            if IsSettings then
                ...
                Tween(Settings_Window_Cool_Snake_Case, {BackgroundTransparency = 0.1}, 0.2)
                Tween(SetScale, {Scale = MainScale.Scale}, 0.2).Completed:Wait()
            else
                Main_Window_Cool_Snake_Case.Visible = true
                Tween(Main_Window_Cool_Snake_Case, {BackgroundTransparency = 0.1}, 0.2)
                Tween(MainScale, {Scale = MainScale.Scale}, 0.2).Completed:Wait()
            end
        else
            local tScale = MainScale.Scale * 0.9
            if IsSettings then
                Tween(Settings_Window_Cool_Snake_Case, {BackgroundTransparency = 1}, 0.15)
                Tween(SetScale, {Scale = tScale}, 0.15).Completed:Wait()
                Settings_Window_Cool_Snake_Case.Visible = false
            else
                Tween(Main_Window_Cool_Snake_Case, {BackgroundTransparency = 1}, 0.15)
                Tween(MainScale, {Scale = tScale}, 0.15).Completed:Wait()
    ...
    local function SwitchToSettings()
        if animating then return end
        animating = true
        Settings_Window_Cool_Snake_Case.Position = Main_Window_Cool_Snake_Case.Position
        Settings_Window_Cool_Snake_Case.Size = Main_Window_Cool_Snake_Case.Size
        Tween(MainScale, {Scale = MainScale.Scale * 0.95}, 0.15).Completed:Wait()
        Main_Window_Cool_Snake_Case.Visible = false
        Settings_Window_Cool_Snake_Case.Visible = true
        Tween(Settings_Window_Cool_Snake_Case, {BackgroundTransparency = 0.1}, 0)
        SetScale.Scale = MainScale.Scale
        IsSettings = true
    ...
    local function SwitchToMain()
        if animating then return end
        animating = true
        Main_Window_Cool_Snake_Case.Position = Settings_Window_Cool_Snake_Case.Position
        Main_Window_Cool_Snake_Case.Size = Settings_Window_Cool_Snake_Case.Size
        Tween(SetScale, {Scale = SetScale.Scale * 0.95}, 0.15).Completed:Wait()
        Settings_Window_Cool_Snake_Case.Visible = false
        Main_Window_Cool_Snake_Case.Visible = true
        Tween(Main_Window_Cool_Snake_Case, {BackgroundTransparency = 0.1}, 0)
        MainScale.Scale = SetScale.Scale
        IsSettings = false
        Library._IsSettings = false
        animating = false
    end
    ...
    local MenuBindConnection = UserInputService.InputBegan:Connect(function(input, gp)
        if not gp and input.KeyCode == Config_Cool_Snake_Case.Keybind then
    ...
    MainPages.Size = UDim2.new(1, -Sidebar_Width_Cool_Snake_Case, 1, 0)
    MainPages.Position = UDim2.new(0, Sidebar_Width_Cool_Snake_Case, 0, 0)
    ...
    local function PopulateSettings()
        ...
        SetPage.Size = UDim2.new(1, -Sidebar_Width_Cool_Snake_Case - 20, 1, -20)
        SetPage.Position = UDim2.new(0, Sidebar_Width_Cool_Snake_Case + 10, 0, 10)
        ...
        SetPage.Active = true
        SetPage.Parent = Settings_Window_Cool_Snake_Case
        RegisterTheme(SetPage, "ScrollBar")
        local ListLayout = Instance.new("UIListLayout")
        ListLayout.SortOrder = Enum.SortOrder.LayoutOrder
        ListLayout.Padding = UDim.new(0, 10)
        ListLayout.Parent = SetPage
        local Set_Padding = Instance.new("UIPadding")
        Set_Padding.PaddingBottom = UDim.new(0, 20)
        Set_Padding.Parent = SetPage
        local MenuSec = WindowObj:CreateRawSection("Menu Settings", SetPage)
        MenuSec:Button("Unload UI", "Destroys the Hub", function()
            Library:Unload()
        end)
        local keybindBtn
        keybindBtn = MenuSec:Button("Menu Keybind: " .. tostring(Config_Cool_Snake_Case.Keybind.Name), "Change the open/close key", function()
            keybindBtn.Text = "Press any key..."
            local conn
            conn = UserInputService.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Keyboard then
                    if input.KeyCode == Enum.KeyCode.Backspace or input.KeyCode == Enum.KeyCode.Delete then
                        Config_Cool_Snake_Case.Keybind = Enum.KeyCode.LeftControl
                    elseif input.KeyCode ~= Enum.KeyCode.Escape and input.KeyCode ~= Enum.KeyCode.Unknown then
                        Config_Cool_Snake_Case.Keybind = input.KeyCode
                    end
                    keybindBtn.Text = "Menu Keybind: " .. tostring(Config_Cool_Snake_Case.Keybind.Name)
                    Library:Notify("Settings", "Menu keybind set to " .. tostring(Config_Cool_Snake_Case.Keybind.Name), 2)
                    conn:Disconnect()
                end
            end)
        end)
    ...
    function WindowObj:Tab(name, iconId)
        ...
        local Page_Padding = Instance.new("UIPadding")
        Page_Padding.PaddingBottom = UDim.new(0, 40)
        Page_Padding.Parent = Page
        ...
        local function Update_Canvas_Size()
            Page.CanvasSize = UDim2.new(0, 0, 0, math.max(LeftList.AbsoluteContentSize.Y, RightList.AbsoluteContentSize.Y) + 30)
        end
