...
function Library:CreateWindow(options)
    ...
    function WindowObj:CreateRawSection(text, parent)
        ...
        function Section:Toggle(text, flag, default, tooltipText, callback)
            ...
            function ToggleObj:Keybind() end
            return ToggleObj
        end

        function Section:Keybind(text, flag, default, tooltipText, callback)
            local keyVal = default or Enum.KeyCode.Unknown
            local mode = "Toggle"
            Library.Defaults[flag] = keyVal
            Library.Flags[flag] = keyVal
            ...
            ModeGui.Size = UDim2.new(0, 80, 0, 40)
            ModeGui.Position = UDim2.new(1, -70, 0, 25)
            ...
            KeyBtn.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton2 then
                    ...
                    if ModeGui.Visible and Content then
            ...
            table.insert(Library.Connections, UserInputService.InputBegan:Connect(function(input, gp)
                if not gp and input.KeyCode == keyVal and keyVal ~= Enum.KeyCode.Unknown then
                    if mode == "Toggle" then
                        state = not state
                        callback(state)
                        Library:UpdateKeybindList(text, keyVal.Name, state, mode)
            ...
            table.insert(Library.Connections, UserInputService.InputEnded:Connect(function(input, gp)
                if not gp and input.KeyCode == keyVal and keyVal ~= Enum.KeyCode.Unknown then
                    if mode == "Hold" then
                        state = false
                        callback(state)
