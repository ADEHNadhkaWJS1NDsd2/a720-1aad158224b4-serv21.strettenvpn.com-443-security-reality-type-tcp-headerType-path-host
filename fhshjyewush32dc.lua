...
local function Refresh_Layout_Coordinates()
    ...
    for _, Slider_Data in ipairs(Interface_Manager.Sliders) do
        ...
        local Current_Value = Configuration[Slider_Data.Config_Key] or Slider_Data.Min_Bound
            ...
            if Ball_Folder then
                for _, Ball_Instance in ipairs(Ball_Folder:GetChildren()) do
                    if Ball_Instance:GetAttribute("realBall") then
                        ...
                        local Current_Velocity_Magnitude = Get_Vector_Magnitude(Parry_State.Ball.Velocity)
                        Parry_State.Ball.Speed    = Current_Velocity_Magnitude
                        ...
                        if Singularity_Cape then
                            ...
                            if Configuration.Auto_Parry and Is_Targeting_Local_Player and not Block_All and Parry_State.Ball.Speed >= (Configuration.Min_Threat_Speed or 5) then
                                local Final_Threshold = Parry_Threshold_Range
            ...
            if Training_Folder then
                for _, Training_Ball in ipairs(Training_Folder:GetChildren()) do
                    if Training_Ball:GetAttribute("realBall") then
                        ...
                        local Training_Threshold = Compute_Parry_Threshold(
                            Training_Ball,
                            Training_Ball_Speed,
                            Player_State.Entity.Server_Position,
                            Player_State.Entity.Velocity,
                            Player_State.Entity.Ping,
                            Distance_To_Training_Ball,
                            Delta_Time
                        )
                        if Configuration.Auto_Curve then
