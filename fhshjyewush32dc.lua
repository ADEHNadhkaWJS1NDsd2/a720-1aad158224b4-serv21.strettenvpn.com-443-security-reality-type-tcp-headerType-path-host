...
function Library:CreateWindow(options)
    ...
    local autoSaveConn = RunService.RenderStepped:Connect(function()
        if Library.FlagsChanged and (tick() - lastAutoSave >= 2) then
            Library.FlagsChanged = false
            lastAutoSave = tick()
    ...
    function WindowObj:CreateRawSection(text, parent)
        ...
        function Section:Toggle(text, flag, default, tooltipText, callback)
            ...
            if Library.Flags[flag] ~= nil then toggled = Library.Flags[flag] end
            Library.Defaults[flag] = default or false
            Library.Flags[flag] = toggled
            Library.FlagsChanged = true
            ...
            function ToggleObj:AddSlider(txt, sflag, min, max, def, cb)
                ...
                if Library.Flags[sflag] ~= nil then val = Library.Flags[sflag] end
        ...
        function Section:Keybind(text, flag, defaultKey, mode, tooltipText, callback)
            ...
            if Library.Flags[flag] ~= nil and type(Library.Flags[flag]) == "table" then
                key = Library.Flags[flag].Key or key
                kMode = Library.Flags[flag].Mode or kMode
            end
            ...
            Library.Signals[flag] = function(val)
                if type(val) == "table" and val.Key and val.Mode then
                    key = val.Key
                    kMode = val.Mode
                    KeyBtn.Text = "[" .. key.Name .. "]"
                    Library:UpdateKeybindList(text, key.Name, true, kMode)
        ...
        function Section:TextBox(text, flag, placeholder, tooltipText, callback)
            local valText = default or ""
            if Library.Flags[flag] ~= nil then valText = Library.Flags[flag] end
            Library.Defaults[flag] = valText
            Library.Flags[flag] = valText
            Library.FlagsChanged = true
            ...
            task.spawn(function() callback(valText) end)
            ApplyTooltip(Frame, tooltipText)
        end
        ...
        function Section:Dropdown(text, flag, options, default, tooltipText, callback, customParent, isMulti)
            ...
            if Library.Flags[flag] ~= nil then selected = Library.Flags[flag] end
        ...
        function Section:ColorPicker(text, flag, default, tooltipText, callback)
            ...
            if Library.Flags[flag] ~= nil then
                if type(Library.Flags[flag]) == "string" then 
                    color = Color3.fromHex(Library.Flags[flag])
                else
