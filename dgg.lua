            ...
            function Elements:KeybindCreate(Name, Flag, Default, Tooltip, Callback)
                ...
                KeybindButton.MouseButton2Click:Connect(function()
                    ...
                    for i, M in ipairs(Modes) do
                        if M == KeybindData.Mode then CurrentIndex = i break end
                    end
                    ...
                    task.delay(0.15, function()
                        if KeybindButton and KeybindButton.Parent then
                            AnimateElement(KeybindButtonStroke, {Color = ColorsTable.borderColor}, 0.2)
                        end
                    end)
                end)
            ...
            function Elements:MultiDropdownCreate(Name, Flag, Options, Default, Tooltip, Callback)
                ...
                function MultiApi:Refresh(NewOptions)
                    ...
                    for _, Option in ipairs(CurrentOptions) do
                        ...
                        OptionChecks[Option] = CheckFrame
                        ...
                        OptionButton.MouseButton1Click:Connect(function()
                            ...
                            for I, V in ipairs(LibraryApi.Flags[Flag]) do
                                if V == Option then
                            ...
                            if not NowSelected then
                                table.insert(LibraryApi.Flags[Flag], Option)
                                NowSelected = true
                            end
                            AnimateElement(CheckFrame, {BackgroundColor3 = NowSelected and ColorsTable.accentColor or ColorsTable.elementBackground}, 0.2)
                            AnimateElement(CheckStroke, {Color = NowSelected and ColorsTable.accentColor or ColorsTable.borderColor}, 0.2)
                            UpdateSelectedText()
                            if Callback then task.spawn(Callback, LibraryApi.Flags[Flag]) end
                        end)
                    end
