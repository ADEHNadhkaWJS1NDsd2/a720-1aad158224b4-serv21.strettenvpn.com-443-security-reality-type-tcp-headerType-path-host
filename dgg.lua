...
local function Save_Configuration(File_Name)
    pcall(function()
        ...
        for Key, Val in pairs(Library_Api.Flags) do
            ...
            elseif type(Val) == "table" and Val.Type == "Multi" then
                Serialized_Data[Key] = {Type = "Multi", Values = Val.Values}
            else
                    ...
                    if type(Val) == "table" then
                        ...
                        elseif Val.Type == "Keybind" then
                            ...
                            if Val.InputType == "KeyCode" then
                                Input_Value = Enum.KeyCode[Val.Name] or Enum.KeyCode.Unknown
                                Library_Api.Flags[Key] = {Type = "KeyCode", Value = Input_Value, Mode = Val.Mode or "Toggle"}
                            elseif Val.InputType == "UserInputType" then
                                Input_Value = Enum.UserInputType[Val.Name] or Enum.UserInputType.None
                                Library_Api.Flags[Key] = {Type = "UserInputType", Value = Input_Value, Mode = Val.Mode or "Toggle"}
                            end
                        elseif Val.Type == "Range" then
                            Library_Api.Flags[Key] = {Min = Val.Min, Max = Val.Max}
                        elseif Val.Type == "Multi" then
                            Library_Api.Flags[Key] = {Type = "Multi", Values = Val.Values or {}}
                        end
                    else
...
function Library_Api:Create_Window(Window_Name)
    ...
    Apply_Acrylic_Effect(Main_Background, 0.88741, UDim.new(0, 8))
    ...
    function Window_Context:Tab_Create(Tab_Name, Icon_Id, Is_Bottom)
        ...
        if Is_Bottom then
            ...
            Set_Color(Avatar_Stroke, "Color", "accent_color")
            Avatar_Stroke.Thickness = 2
            Avatar_Stroke.Parent = Tab_Icon
            ...
            local Profile_Stroke = Instance.new("UIStroke")
            Set_Color(Profile_Stroke, "Color", "border_light_color")
            Profile_Stroke.Thickness = 1
            Profile_Stroke.Parent = Tab_Button
        else
        ...
        local function Element_Injector(Target_Container)
            ...
            function Elements:Range_Slider_Create(Name, Flag, Min, Max, Default_Min, Default_Max, Step, Tooltip, Callback)
            ...
            function Elements:Keybind_Create(Name, Flag, Default, Tooltip, Callback)
                if type(Library_Api.Flags[Flag]) ~= "table" or not Library_Api.Flags[Flag].Value then
                    local Old = Library_Api.Flags[Flag]
                    if typeof(Old) == "EnumItem" then
                        if Old.EnumType == Enum.KeyCode then
                            Library_Api.Flags[Flag] = {Type = "KeyCode", Value = Old, Mode = "Toggle"}
                        else
                            Library_Api.Flags[Flag] = {Type = "UserInputType", Value = Old, Mode = "Toggle"}
                        end
                    else
                        Library_Api.Flags[Flag] = {Type = "KeyCode", Value = Default or Enum.KeyCode.Unknown, Mode = "Toggle"}
                    end
                end

                local Keybind_Data = Library_Api.Flags[Flag]
                local Is_Listening = false
                local Modes = {"Hold", "Toggle", "Always"}

                local function Get_Input_Display()
                    if Keybind_Data.Type == "KeyCode" then
                        return Keybind_Data.Value.Name or "None"
                    elseif Keybind_Data.Type == "UserInputType" then
                        if Keybind_Data.Value == Enum.UserInputType.MouseButton1 then return "Mouse1"
                        elseif Keybind_Data.Value == Enum.UserInputType.MouseButton2 then return "Mouse2"
                        elseif Keybind_Data.Value == Enum.UserInputType.MouseButton3 then return "Mouse3"
                        else return Keybind_Data.Value.Name end
                    end
                    return "None"
                end
                ...
                Keybind_Label.Position = UDim2.new(0, 2, 0, 0)
                ...
                Keybind_Button.Size = UDim2.new(0, 95, 0, 22)
                Keybind_Button.Position = UDim2.new(1, -99, 0.5, -11)
                Set_Color(Keybind_Button, "BackgroundColor3", "element_background")
                Keybind_Button.BackgroundTransparency = 0.21847
                Keybind_Button.Text = "[ " .. Get_Input_Display() .. " ] " .. Keybind_Data.Mode
                ...
                Keybind_Button.MouseButton2Click:Connect(function()
                    for _, Child in ipairs(Screen_Gui:GetChildren()) do
                        if Child.Name == "Keybind_Mode_Menu" or Child.Name == "Keybind_Close_Catcher" then Child:Destroy() end
                    end

                    local Close_Catcher = Instance.new("TextButton")
                    Close_Catcher.Name = "Keybind_Close_Catcher"
                    Close_Catcher.Size = UDim2.new(1, 0, 1, 0)
                    Close_Catcher.BackgroundTransparency = 1
                    Close_Catcher.Text = ""
                    Close_Catcher.ZIndex = 3499
                    Close_Catcher.Parent = Screen_Gui

                    local Context_Menu = Instance.new("Frame")
                    Context_Menu.Name = "Keybind_Mode_Menu"
                    Context_Menu.Size = UDim2.new(0, 118, 0, 92)
                    Context_Menu.Position = UDim2.new(0, Keybind_Button.AbsolutePosition.X - 10, 0, Keybind_Button.AbsolutePosition.Y + Keybind_Button.AbsoluteSize.Y + 6)
                    Set_Color(Context_Menu, "BackgroundColor3", "element_background")
                    Context_Menu.BackgroundTransparency = 0.08
                    Context_Menu.BorderSizePixel = 0
                    Context_Menu.ZIndex = 3500
                    Context_Menu.Parent = Screen_Gui
                    
                    local Menu_Corner = Instance.new("UICorner")
                    Menu_Corner.CornerRadius = UDim.new(0, 8)
                    Menu_Corner.Parent = Context_Menu
                    
                    local Menu_Stroke = Instance.new("UIStroke")
                    Set_Color(Menu_Stroke, "Color", "accent_color")
                    Menu_Stroke.Thickness = 1.5
                    Menu_Stroke.Transparency = 0.3
                    Menu_Stroke.Parent = Context_Menu

                    Apply_Acrylic_Effect(Context_Menu, 0.92, UDim.new(0, 8))

                    local Menu_Layout = Instance.new("UIListLayout")
                    Menu_Layout.SortOrder = Enum.SortOrder.LayoutOrder
                    Menu_Layout.Padding = UDim.new(0, 3)
                    Menu_Layout.Parent = Context_Menu

                    local Title_Label = Instance.new("TextLabel")
                    Title_Label.Size = UDim2.new(1, 0, 0, 22)
                    Title_Label.BackgroundTransparency = 1
                    Title_Label.Text = "Select Mode"
                    Set_Color(Title_Label, "TextColor3", "text_white_color")
                    Title_Label.TextSize = 11
                    Title_Label.Font = Bold_Font
                    Title_Label.ZIndex = 3501
                    Title_Label.Parent = Context_Menu

                    Close_Catcher.MouseButton1Click:Connect(function()
                        Context_Menu:Destroy()
                        Close_Catcher:Destroy()
                    end)

                    for _, Mode_Name in ipairs(Modes) do
                        local Mode_Btn = Instance.new("TextButton")
                        Mode_Btn.Size = UDim2.new(1, -8, 0, 24)
                        Mode_Btn.Position = UDim2.new(0, 4, 0, 0)
                        Mode_Btn.BackgroundTransparency = 1
                        Mode_Btn.Text = Mode_Name
                        Set_Color(Mode_Btn, "TextColor3", Keybind_Data.Mode == Mode_Name and "accent_color" or "text_white_color")
                        Mode_Btn.TextSize = 12
                        Mode_Btn.Font = Main_Font
                        Mode_Btn.ZIndex = 3501
                        Mode_Btn.Parent = Context_Menu
                        
                        local Btn_Corner = Instance.new("UICorner")
                        Btn_Corner.CornerRadius = UDim.new(0, 5)
                        Btn_Corner.Parent = Mode_Btn

                        Mode_Btn.MouseButton1Click:Connect(function()
                            Keybind_Data.Mode = Mode_Name
                            Keybind_Button.Text = "[ " .. Get_Input_Display() .. " ] " .. Keybind_Data.Mode
                            Context_Menu:Destroy()
                            Close_Catcher:Destroy()
                            if Callback then task.spawn(Callback, Keybind_Data) end
                        end)
                        
                        Mode_Btn.MouseEnter:Connect(function()
                            Animate_Element(Mode_Btn, {BackgroundTransparency = 0.25, BackgroundColor3 = Colors_Table.element_hover_background}, 0.12)
                        end)
                        Mode_Btn.MouseLeave:Connect(function()
                            Animate_Element(Mode_Btn, {BackgroundTransparency = 1, BackgroundColor3 = Colors_Table.element_background}, 0.12)
                        end)
                    end
                end)
                ...
                User_Input_Service.InputBegan:Connect(function(Input)
                    if Is_Listening then
                        ...
                        if Input.KeyCode ~= Enum.KeyCode.Unknown and Input.KeyCode ~= Enum.KeyCode.Escape then
                            New_Bind = {Type = "KeyCode", Value = Input.KeyCode}
                        elseif Input.UserInputType == Enum.UserInputType.MouseButton1 or
                               Input.UserInputType == Enum.UserInputType.MouseButton2 or
                               Input.UserInputType == Enum.UserInputType.MouseButton3 then
                            New_Bind = {Type = "UserInputType", Value = Input.UserInputType}
                        elseif Input.KeyCode == Enum.KeyCode.Escape then
                            New_Bind = {Type = "KeyCode", Value = Enum.KeyCode.Unknown}
                        end
                        if New_Bind then
                            Keybind_Data.Type = New_Bind.Type
                            Keybind_Data.Value = New_Bind.Value
                            Keybind_Button.Text = "[ " .. Get_Input_Display() .. " ] " .. Keybind_Data.Mode
                            Is_Listening = false
                        ...
                        local Matches = false
                        if Keybind_Data.Type == "KeyCode" and Input.KeyCode == Keybind_Data.Value then
                            Matches = true
                        elseif Keybind_Data.Type == "UserInputType" and Input.UserInputType == Keybind_Data.Value then
                            Matches = true
                        end
                        if Matches and Keybind_Data.Value ~= Enum.KeyCode.Unknown then
                            if Callback then task.spawn(Callback, Keybind_Data) end
                        end
                    end
                end)
            ...
            function Elements:Multi_Dropdown_Create(Name, Flag, Options, Default, Tooltip, Callback)
                Library_Api.Flags[Flag] = Library_Api.Flags[Flag] or (Default or {})
                if type(Library_Api.Flags[Flag]) ~= "table" then Library_Api.Flags[Flag] = {} end
                local Is_Open = false
                local Current_Options = Options
                local Multi_Api = {}

                local Multi_Frame = Instance.new("Frame")
                Multi_Frame.Size = UDim2.new(1, 0, 0, 46)
                Multi_Frame.BackgroundTransparency = 1
                Multi_Frame.ClipsDescendants = true
                Multi_Frame.Parent = Target_Container

                local Multi_Label = Instance.new("TextLabel")
                Multi_Label.Size = UDim2.new(1, -10, 0, 14)
                Multi_Label.Position = UDim2.new(0, 2, 0, 0)
                Multi_Label.BackgroundTransparency = 1
                Multi_Label.Text = Name
                Set_Color(Multi_Label, "TextColor3", "text_white_color")
                Multi_Label.TextSize = 12
                Multi_Label.Font = Main_Font
                Multi_Label.TextXAlignment = Enum.TextXAlignment.Left
                Multi_Label.Parent = Multi_Frame

                local Multi_Main_Button = Instance.new("TextButton")
                Multi_Main_Button.Size = UDim2.new(1, -4, 0, 24)
                Multi_Main_Button.Position = UDim2.new(0, 2, 0, 20)
                Set_Color(Multi_Main_Button, "BackgroundColor3", "element_background")
                Multi_Main_Button.BackgroundTransparency = 0.21847
                Multi_Main_Button.Text = ""
                Multi_Main_Button.AutoButtonColor = false
                Multi_Main_Button.Parent = Multi_Frame
                
                local Multi_Main_Button_Corner = Instance.new("UICorner")
                Multi_Main_Button_Corner.CornerRadius = UDim.new(0, 4)
                Multi_Main_Button_Corner.Parent = Multi_Main_Button
                
                local Multi_Main_Button_Stroke = Instance.new("UIStroke")
                Set_Color(Multi_Main_Button_Stroke, "Color", "border_color")
                Multi_Main_Button_Stroke.Parent = Multi_Main_Button

                local Selected_Text_Label = Instance.new("TextLabel")
                Selected_Text_Label.Size = UDim2.new(1, -30, 1, 0)
                Selected_Text_Label.Position = UDim2.new(0, 8, 0, 0)
                Selected_Text_Label.BackgroundTransparency = 1
                Set_Color(Selected_Text_Label, "TextColor3", "text_dark_color")
                Selected_Text_Label.TextSize = 11
                Selected_Text_Label.Font = Main_Font
                Selected_Text_Label.TextXAlignment = Enum.TextXAlignment.Left
                Selected_Text_Label.Parent = Multi_Main_Button

                local Multi_Arrow_Icon = Instance.new("ImageLabel")
                Multi_Arrow_Icon.Size = UDim2.new(0, 14, 0, 14)
                Multi_Arrow_Icon.Position = UDim2.new(1, -22, 0.5, -7)
                Multi_Arrow_Icon.BackgroundTransparency = 1
                Multi_Arrow_Icon.Image = "rbxassetid://6031090656"
                Set_Color(Multi_Arrow_Icon, "ImageColor3", "text_dark_color")
                Multi_Arrow_Icon.Parent = Multi_Main_Button

                local Multi_Option_List_Frame = Instance.new("ScrollingFrame")
                Multi_Option_List_Frame.Size = UDim2.new(1, -4, 0, 0)
                Multi_Option_List_Frame.Position = UDim2.new(0, 2, 0, 48)
                Set_Color(Multi_Option_List_Frame, "BackgroundColor3", "element_background")
                Multi_Option_List_Frame.BackgroundTransparency = 0.21847
                Multi_Option_List_Frame.BorderSizePixel = 0
                Multi_Option_List_Frame.ScrollBarThickness = 2
                Set_Color(Multi_Option_List_Frame, "ScrollBarImageColor3", "accent_color")
                Multi_Option_List_Frame.ClipsDescendants = true
                Multi_Option_List_Frame.Parent = Multi_Frame
                
                local Multi_Option_List_Corner = Instance.new("UICorner")
                Multi_Option_List_Corner.CornerRadius = UDim.new(0, 4)
                Multi_Option_List_Corner.Parent = Multi_Option_List_Frame
                
                local Multi_Option_List_Stroke = Instance.new("UIStroke")
                Set_Color(Multi_Option_List_Stroke, "Color", "border_color")
                Multi_Option_List_Stroke.Transparency = 1
                Multi_Option_List_Stroke.Parent = Multi_Option_List_Frame

                local Multi_Option_List_Layout = Instance.new("UIListLayout")
                Multi_Option_List_Layout.SortOrder = Enum.SortOrder.LayoutOrder
                Multi_Option_List_Layout.Parent = Multi_Option_List_Frame

                local function Update_Selected_Text()
                    local Sel = Library_Api.Flags[Flag]
                    if #Sel == 0 then
                        Selected_Text_Label.Text = "None"
                    elseif #Sel == 1 then
                        Selected_Text_Label.Text = Sel[1]
                    elseif #Sel <= 2 then
                        Selected_Text_Label.Text = table.concat(Sel, ", ")
                    else
                        Selected_Text_Label.Text = #Sel .. " selected"
                    end
                end
                Update_Selected_Text()

                local function Toggle_Multi_State()
                    Is_Open = not Is_Open
                    local Max_List_Height = math.min(#Current_Options * 24 + 4, 160)
                    local Target_List_Height = Is_Open and Max_List_Height or 0
                    Animate_Element(Multi_Main_Button_Stroke, {Color = Is_Open and Colors_Table.accent_color or Colors_Table.border_color}, 0.3)
                    Animate_Element(Multi_Arrow_Icon, {Rotation = Is_Open and 180 or 0, ImageColor3 = Is_Open and Colors_Table.accent_color or Colors_Table.text_dark_color}, 0.3)
                    Animate_Element(Multi_Option_List_Frame, {Size = UDim2.new(1, -4, 0, Target_List_Height)}, 0.3)
                    Animate_Element(Multi_Option_List_Stroke, {Transparency = Is_Open and 0 or 1}, 0.3)
                    Animate_Element(Multi_Frame, {Size = UDim2.new(1, 0, 0, 46 + Target_List_Height + (Is_Open and 4 or 0))}, 0.3)
                end

                Multi_Main_Button.MouseEnter:Connect(function()
                    Show_Tooltip(Tooltip)
                    if not Is_Open then Animate_Element(Multi_Main_Button_Stroke, {Color = Colors_Table.border_light_color}, 0.25) end
                end)
                Multi_Main_Button.MouseLeave:Connect(function()
                    Show_Tooltip("")
                    if not Is_Open then Animate_Element(Multi_Main_Button_Stroke, {Color = Colors_Table.border_color}, 0.25) end
                end)
                Multi_Main_Button.MouseButton1Click:Connect(Toggle_Multi_State)

                function Multi_Api:Refresh(New_Options)
                    Current_Options = New_Options or Current_Options
                    for _, Child in ipairs(Multi_Option_List_Frame:GetChildren()) do
                        if Child:IsA("TextButton") then Child:Destroy() end
                    end
                    for _, Option in ipairs(Current_Options) do
                        local Option_Button = Instance.new("TextButton")
                        ...
                        Option_Button.BackgroundTransparency = 1
                        Option_Button.Text = ""
                        Option_Button.Parent = Multi_Option_List_Frame

                        local Check_Frame = Instance.new("Frame")
                        Check_Frame.Size = UDim2.new(0, 14, 0, 14)
                        Check_Frame.Position = UDim2.new(0, 8, 0.5, -7)
                        local Is_Selected = false
                        for _, V in ipairs(Library_Api.Flags[Flag]) do
                            if V == Option then Is_Selected = true break end
                        end
                        Set_Color(Check_Frame, "BackgroundColor3", Is_Selected and "accent_color" or "element_background")
                        Check_Frame.BackgroundTransparency = 0.21847
                        Check_Frame.Parent = Option_Button
                        
                        local Check_Corner = Instance.new("UICorner")
                        Check_Corner.CornerRadius = UDim.new(0, 3)
                        Check_Corner.Parent = Check_Frame
                        
                        local Check_Stroke = Instance.new("UIStroke")
                        Set_Color(Check_Stroke, "Color", Is_Selected and "accent_color" or "border_color")
                        Check_Stroke.Parent = Check_Frame

                        local Option_Label = Instance.new("TextLabel")
                        Option_Label.Size = UDim2.new(1, -30, 1, 0)
                        Option_Label.Position = UDim2.new(0, 28, 0, 0)
                        Option_Label.BackgroundTransparency = 1
                        Option_Label.Text = Option
                        Set_Color(Option_Label, "TextColor3", "text_dark_color")
                        Option_Label.TextSize = 12
                        Option_Label.Font = Main_Font
                        Option_Label.TextXAlignment = Enum.TextXAlignment.Left
                        Option_Label.Parent = Option_Button
                        ...
                        Option_Button.MouseEnter:Connect(function()
                            Animate_Element(Option_Button, {BackgroundTransparency = 0.21847}, 0.25)
                            Animate_Element(Option_Label, {TextColor3 = Colors_Table.text_white_color}, 0.25)
                        end)
                        Option_Button.MouseLeave:Connect(function()
                            Animate_Element(Option_Button, {BackgroundTransparency = 1}, 0.25)
                            Animate_Element(Option_Label, {TextColor3 = Colors_Table.text_dark_color}, 0.25)
                        end)

                        Option_Button.MouseButton1Click:Connect(function()
                            local Is_Currently_Selected = false
                            for _, V in ipairs(Library_Api.Flags[Flag]) do
                                if V == Option then Is_Currently_Selected = true break end
                            end
                            
                            if Is_Currently_Selected then
                                for I = #Library_Api.Flags[Flag], 1, -1 do
                                    if Library_Api.Flags[Flag][I] == Option then
                                        table.remove(Library_Api.Flags[Flag], I)
                                        break
                                    end
                                end
                                Animate_Element(Check_Frame, {BackgroundColor3 = Colors_Table.element_background}, 0.2)
                                Animate_Element(Check_Stroke, {Color = Colors_Table.border_color}, 0.2)
                            else
                                table.insert(Library_Api.Flags[Flag], Option)
                                Animate_Element(Check_Frame, {BackgroundColor3 = Colors_Table.accent_color}, 0.2)
                                Animate_Element(Check_Stroke, {Color = Colors_Table.accent_color}, 0.2)
                            end
                            Update_Selected_Text()
                            if Callback then task.spawn(Callback, Library_Api.Flags[Flag]) end
                        end)
                    end
                    Multi_Option_List_Frame.CanvasSize = UDim2.new(0, 0, 0, #Current_Options * 24)
                    Update_Selected_Text()
                end

                Multi_Api:Refresh(Current_Options)
                return Multi_Api
            end

            function Elements:Color_Picker_Create(Name, Flag, Default, Tooltip, Callback)
                ...
                Color_Preview_Button.MouseButton1Click:Connect(function()
                    local Current_Color = Library_Api.Flags[Flag]
                    Hue, Saturation, Value = Current_Color:ToHSV()
                    Saturation_Value_Map.ImageColor3 = Color3.fromHSV(Hue, 1, 1)
                    Saturation_Value_Map_Cursor.Position = UDim2.new(Saturation, 0, 1 - Value, 0)
                    Hue_Map_Cursor.Position = UDim2.new(Hue, 0, 0.5, 0)
                    Color_Preview_Button.BackgroundColor3 = Current_Color
            ...
            function Elements:Button_Create(Name, Tooltip, Callback)
                ...
                Action_Button.MouseEnter:Connect(function()
                    ...
                    Animate_Element(Action_Button_Stroke, {Color = Colors_Table.border_light_color}, 0.25)
                end)
            ...
            function Elements:Sub_Button_Create(Name, Tooltip, Callback)
    ...
    for Key, Default_Color in pairs(Colors_Table) do
        if not Key:match("notification") and not Key:match("gradient") then
            Theme_Section:Color_Picker_Create(Hex_Format(Key), "Theme_Color_" .. Key, Default_Color, "Change " .. Hex_Format(Key), function(New_Color)
    ...
    User_Input_Service.InputBegan:Connect(function(Input, Game_Processed_Event)
        if not Game_Processed_Event then
            local Menu_Bind = Library_Api.Flags["Menu_Toggle_Key"]
            local Matches = false
            if Menu_Bind and Menu_Bind.Type == "KeyCode" and Input.KeyCode == Menu_Bind.Value then
                Matches = true
            elseif Menu_Bind and Menu_Bind.Type == "UserInputType" and Input.UserInputType == Menu_Bind.Value then
                Matches = true
            end
            if Matches then
                Main_Background.Visible = not Main_Background.Visible
