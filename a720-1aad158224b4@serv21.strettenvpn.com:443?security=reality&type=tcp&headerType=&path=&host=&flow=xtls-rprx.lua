...
local Config = {
    AutoParry = false, AutoSpam = false, SpamSensitivity = 50, LobbyParry = false, TriggerbotEnabled = false,
    NoClickOnBallSpawn = true, DotProtect = true, MinThreatSpeed = 5, ParryCooldown = 0, PingMultiplier = 1.0,
    SpeedDivisorBase = 2.4, SpeedDivisorMultiplier = 0.002, CappedSpeed = 9999, SpeedDivisionFactor = 1.1,
    PingSampleCount = 50, DotMinSpeed = 100.0, DotThreshold = 0.820, DotDistanceThreshold = 30.0,
    BaseMinParryAccuracy = 25.0, ParryKeybind = "None", SpamKeybind = "None", TriggerbotKeybind = "None",
    HideKeybind = "Esc", ParryMethod = "Click", RenderBallStats = false, ParryBindMode = "Toggle",
    SpamBindMode = "Toggle", TriggerbotBindMode = "Toggle", ShowHotkeyList = true, AutoSave = true,
    AutoLoad = true, ThemePreset = "Nightfall", ConfigName = "default", SelectedConfig = "default",
    SaveConfigAction = false, LoadConfigAction = false
}

local Curve_Min_Speed_Threshold = 75.0
local Curve_Dot_Limit = 0.65

local function IsPlayerInFolder(folderName)
    local folder = Workspace:FindFirstChild(folderName)
    if not folder then return false end
    local character = LocalPlayer.Character
    if character and character.Parent == folder then return true end
    if folder:FindFirstChild(LocalPlayer.Name) then return true end
    return false
end

local function GetPlayerFolderMode()
    if IsPlayerInFolder("Dead") then return "Dead" end
    if IsPlayerInFolder("Alive") then return "Alive" end
    return "Alive"
end

local function IsAliveMode() return GetPlayerFolderMode() == "Alive" end
local function IsDeadMode() return GetPlayerFolderMode() == "Dead" end

local RuntimeState = {
    LastParry = 0, Target = nil, PingHistory = {}, SpamExpiration = 0, SpamModeActive = false,
    ConsecutiveParries = 0, SpamCooldown = 0, ScheduledTrigger = 0, AerodynamicActive = false,
    AerodynamicTime = 0, LastBallSpawn = 0, TargetSpeed = 0, TargetDistance = 0, TargetDot = 0
}

local function Clamp(v, a, b) return math.max(a, math.min(b, v)) end
local function Round(n) return math.floor(n + 0.5) end
local function Vec2(x, y) return Vector2.new(Round(x), Round(y)) end
...
local function SnapValue(val, step) return step and math.floor((val / step) + 0.5) * step or val end
...
local function HSVToColor3(h, s, v)
    ...
    if hp >= 0 and hp < 1 then r1, g1, b1 = c, x, 0
    elseif hp >= 1 and hp < 2 then r1, g1, b1 = x, c, 0
    elseif hp >= 2 and hp < 3 then r1, g1, b1 = 0, c, x
    elseif hp >= 3 and hp < 4 then r1, g1, b1 = 0, x, c
    elseif hp >= 4 and hp < 5 then r1, g1, b1 = x, 0, c
...
local function Color3ToHSV(color)
    if not color then return 0, 0, 1 end
    ...
    local h2 = 0
    if delta > 0 then
        if maxc == r then h2 = ((g - b) / delta) % 6
        elseif maxc == g then h2 = ((b - r) / delta) + 2
        else h2 = ((r - g) / delta) + 4 end
        h2 = h2 / 6
    end
...
local function CreateGridSquares(count)
    local grid = {}
    for i = 1, count do grid[i] = CreateDrawing("Square", {Filled = true, Transparency = 1, Visible = false}) end
    return grid
end

local function UpdateGridSquares(grid, cols, rows, x, y, w, h, colorFunc, visible)
    if not visible or w <= 0 or h <= 0 then
        for _, sq in ipairs(grid) do sq.Visible = false end
        return
    end
    local cellW = w / cols
    local cellH = h / rows
...
local function GetStepDecimals(step)
    if type(step) ~= "number" then return 0 end
...
local function ParseSliderInput(textValue)
    ...
    if cleaned == "" or cleaned == "-" or cleaned == "." or cleaned == "-." then return nil end
...
local function NormalizeKeybindValue(value)
    if type(value) == "number" then return KeyCodes[value] or tostring(value) end
...
local function NormalizeBindMode(mode)
    ...
    if string.lower(mode) == "hold" then return "Hold" end
    return "Toggle"
end

local function GetModeFlagFromBindFlag(bindFlag)
    if bindFlag == "ParryKeybind" then return "ParryBindMode"
    elseif bindFlag == "SpamKeybind" then return "SpamBindMode"
    elseif bindFlag == "TriggerbotKeybind" then return "TriggerbotBindMode" end
end

local function GetToggleFlagFromBindFlag(bindFlag)
    if bindFlag == "ParryKeybind" then return "AutoParry"
    elseif bindFlag == "SpamKeybind" then return "AutoSpam"
    elseif bindFlag == "TriggerbotKeybind" then return "TriggerbotEnabled" end
end

local function SetBindMode(bindFlag, mode)
    local modeFlag = GetModeFlagFromBindFlag(bindFlag)
    if modeFlag then Config[modeFlag] = NormalizeBindMode(mode) end
end
...
local function SetBindTargetState(bindFlag, state)
    ...
    if toggleFlag then Config[toggleFlag] = state and true or false end
end

local function HandleBindPress(bindFlag)
    if NormalizeKeybindValue(Config[bindFlag]) == "None" then return end
    if GetBindMode(bindFlag) == "Hold" then
        SetBindTargetState(bindFlag, true)
    else
        local toggleFlag = GetToggleFlagFromBindFlag(bindFlag)
        if toggleFlag then Config[toggleFlag] = not Config[toggleFlag] end
    end
end

local function HandleBindRelease(bindFlag)
    if NormalizeKeybindValue(Config[bindFlag]) == "None" then return end
    if GetBindMode(bindFlag) == "Hold" then SetBindTargetState(bindFlag, false) end
end
...
CreateDrawing = function(className, properties)
    ...
    for propName, propValue in pairs(properties) do
        pcall(function() drawing[propName] = propValue end)
    end
...
local function CreateKeybindImageIcon(size)
    size = size or 18
    local function square() return CreateDrawing("Square", {Filled = true, Color = Color3.new(1, 1, 1), Transparency = 1, Visible = false}) end
    local function circle() return CreateDrawing("Circle", {Filled = true, Color = Color3.new(1, 1, 1), Transparency = 1, NumSides = 18, Radius = 1, Visible = false}) end
    return {
        IsImage = false, Size = size,
        Parts = {
            Main = square(), Top = square(), Bottom = square(), Left = square(), Right = square(),
            TL = circle(), TR = circle(), BL = circle(), BR = circle(), Inner = square(),
            Dot1 = circle(), Dot2 = circle(), Dot3 = circle(),
...
local function UpdateKeybindImageIcon(icon, position, color, visible)
    ...
    if icon.IsImage and icon.Image then
        ...
        pcall(function() image.Position = position end)
        pcall(function() image.Size = Vec2(icon.Size, icon.Size) end)
        pcall(function() image.Color = color end)
        pcall(function() image.Transparency = visible and 1 or 0 end)
        return
    end
...
local function MakeGradientLine(count)
    local items = {}
    for i = 1, count do items[i] = CreateDrawing("Square", {Filled = true, Visible = false}) end
    return items
end
...
local function MakeStripePattern(count, thickness)
    local items = {}
    for i = 1, count do items[i] = CreateDrawing("Line", {Thickness = thickness or 10, Transparency = 0.08, Visible = false}) end
    return items
end

local function UpdateStripePattern(items, rectX, rectY, rectW, rectH, slant, spacing, color, visible)
    for _, line in ipairs(items) do line.Visible = false end
    if not visible or rectW <= 0 or rectH <= 0 then return end
    ...
    for i, line in ipairs(items) do
        ...
        if toX < left or fromX > right then
            ...
            if fromY < top then fromY = top elseif fromY > bottom then fromY = bottom end
            if toY < top then toY = top elseif toY > bottom then toY = bottom end
...
local function GetContextMenuPosition(item)
    ...
    if not item or not item.ButtonPos or not item.ButtonSize then return Vec2(0, 0) end
    local contextX = item.ButtonPos.X
    local contextY = item.ButtonPos.Y + item.ButtonSize.Y + 6
    if contextX + menuW > viewport.X then contextX = item.ButtonPos.X + item.ButtonSize.X - menuW end
    if contextY + menuH > viewport.Y then contextY = item.ButtonPos.Y - menuH - 6 end
...
local Library = {
    ...
    Input = {
        MousePos = Vector2.new(0, 0), Mouse1Down = false, Mouse1Prev = false, Mouse1Clicked = false, Mouse1Released = false,
        Mouse2Down = false, Mouse2Prev = false, Mouse2Clicked = false, KeysDown = {}
    },
    State = {
        ActiveDropdown = nil, ActiveSlider = nil, ActiveSliderInput = nil, ActiveTextbox = nil, ActiveKeybind = nil,
        ActiveColorPicker = nil, ActiveColorDrag = nil, Dragging = false, DragStart = Vector2.new(0, 0), WindowStart = Vector2.new(0, 0),
...
local DefaultNightfallPalette = {
    Background = Color3.new(0.035294, 0.035294, 0.050980), Sidebar = Color3.new(0.050980, 0.050980, 0.066666),
    Section = Color3.new(0.066666, 0.066666, 0.082352), Element = Color3.new(0.090196, 0.090196, 0.105882),
    Hover = Color3.new(0.121568, 0.121568, 0.145098), Outline = Color3.new(0.105882, 0.105882, 0.133333),
    OutlineLight = Color3.new(0.172549, 0.172549, 0.211764), Accent = Color3.new(0.423529, 0.576470, 0.988235),
    Accent2 = Color3.new(0.619607, 0.462745, 0.988235), Text = Color3.new(0.952941, 0.952941, 0.972549),
    SubText = Color3.new(0.541176, 0.541176, 0.580392)
}

local ThemePresets = {
    Nightfall = {
        Background = DefaultNightfallPalette.Background, Sidebar = DefaultNightfallPalette.Sidebar,
        Section = DefaultNightfallPalette.Section, Element = DefaultNightfallPalette.Element,
        Hover = DefaultNightfallPalette.Hover, Outline = DefaultNightfallPalette.Outline,
        OutlineLight = DefaultNightfallPalette.OutlineLight, Accent = DefaultNightfallPalette.Accent,
        Accent2 = DefaultNightfallPalette.Accent2, Text = DefaultNightfallPalette.Text, SubText = DefaultNightfallPalette.SubText
    },
    Bloodmoon = {
        Background = Color3.fromRGB(20, 8, 12), Sidebar = Color3.fromRGB(28, 10, 16), Section = Color3.fromRGB(36, 12, 20),
        Element = Color3.fromRGB(46, 14, 24), Hover = Color3.fromRGB(58, 18, 32), Outline = Color3.fromRGB(70, 28, 40),
        OutlineLight = Color3.fromRGB(96, 42, 56), Accent = Color3.fromRGB(235, 72, 96), Accent2 = Color3.fromRGB(255, 128, 164),
        Text = Color3.fromRGB(246, 238, 242), SubText = Color3.fromRGB(176, 150, 158)
    },
    Ocean = {
        Background = Color3.fromRGB(8, 14, 24), Sidebar = Color3.fromRGB(10, 18, 32), Section = Color3.fromRGB(12, 24, 40),
        Element = Color3.fromRGB(16, 30, 50), Hover = Color3.fromRGB(20, 40, 64), Outline = Color3.fromRGB(28, 52, 78),
        OutlineLight = Color3.fromRGB(42, 70, 100), Accent = Color3.fromRGB(74, 168, 255), Accent2 = Color3.fromRGB(98, 220, 255),
        Text = Color3.fromRGB(240, 246, 255), SubText = Color3.fromRGB(148, 170, 196)
    },
    Mint = {
        Background = Color3.fromRGB(10, 18, 16), Sidebar = Color3.fromRGB(12, 24, 20), Section = Color3.fromRGB(14, 30, 24),
        Element = Color3.fromRGB(18, 38, 30), Hover = Color3.fromRGB(24, 48, 38), Outline = Color3.fromRGB(38, 68, 56),
        OutlineLight = Color3.fromRGB(58, 96, 82), Accent = Color3.fromRGB(64, 224, 160), Accent2 = Color3.fromRGB(124, 255, 208),
        Text = Color3.fromRGB(238, 252, 246), SubText = Color3.fromRGB(146, 178, 164)
    }
}
...
local function SaveGlobalSettings()
    ...
    local data = {
        AutoSave = Config.AutoSave and true or false, AutoLoad = Config.AutoLoad and true or false,
        SelectedConfig = tostring(Config.SelectedConfig or Config.ConfigName or "default"), ConfigName = tostring(Config.ConfigName or Config.SelectedConfig or "default")
    }
...
local function LoadGlobalSettings()
    if not (isfile and readfile and isfile(GlobalSettingsPath)) then return false end
    local ok, decoded = pcall(function() return HttpService:JSONDecode(readfile(GlobalSettingsPath)) end)
...
local function GetConfigPath() return "Moonshade_" .. GetConfigBaseName() .. ".json" end
local function GetLayoutPath() return "Moonshade_" .. GetConfigBaseName() .. "_layout.json" end
    ...
    if listfiles then
        ...
        if ok and type(files) == "table" then
            for _, filePath in ipairs(files) do
                ...
                if name and not name:match("_layout$") then add(name) end
            end
        end
    end
    if #names == 0 then add("default") end
    table.sort(names, function(a, b) return tostring(a):lower() < tostring(b):lower() end)
    return names
end

local function GetStripeBackColor() return LerpColor(Library.Palette.Accent, Library.Palette.Background, 0.55) end
local function GetStripeFrontColor() return LerpColor(Library.Palette.Accent2, Library.Palette.Background, 0.45) end

local function ApplyPalette(palette)
    if type(palette) ~= "table" then return end
    for _, key in ipairs(PaletteKeys) do
        if palette[key] then Library.Palette[key] = palette[key] end
    end
end

local function SyncThemeEditorState()
    if not Library or not Library.Tabs then return end
...
local function ApplyThemePreset(themeName, keepAccent)
    ...
    Config.UiAccentColor = {Color = Library.Palette.Accent, Alpha = accentAlpha}
    Config.UiAccent2Color = {Color = Library.Palette.Accent2, Alpha = accent2Alpha}
    SyncThemeEditorState()
    QueueSaveConfig()
end

QueueSaveConfig = function() SaveConfigQueued = true end
...
local function SaveCurrentConfig()
    ...
    local layout = {
        WindowX = Library.TargetPosition.X, WindowY = Library.TargetPosition.Y,
        HotkeysX = Library.HotkeysPosition.X, HotkeysY = Library.HotkeysPosition.Y,
        StatsX = Library.StatsPosition.X, StatsY = Library.StatsPosition.Y
    }
    local ok2, layoutEncoded = pcall(function() return HttpService:JSONEncode(layout) end)
...
local function DeleteNamedConfig(name)
    ...
    local deleted = false
    if delfile then
        if isfile and isfile(cfgPath) then pcall(delfile, cfgPath) deleted = true end
        if isfile and isfile(layoutPath) then pcall(delfile, layoutPath) end
    end
    local available = GetAvailableConfigs()
    local fallback = available[1] or "default"
    if Config.SelectedConfig == target then Config.SelectedConfig = fallback end
    if Config.ConfigName == target then Config.ConfigName = fallback end
    SaveGlobalSettings()
    SaveConfigQueued = false
    return deleted
end
...
local function LoadNamedConfig(name)
    ...
    if name and name ~= "" then Config.ConfigName = tostring(name) end
    ...
    if decoded.AutoLoadConfig ~= nil and decoded.AutoLoad == nil then Config.AutoLoad = decoded.AutoLoadConfig and true or false end
    ...
    if type(decoded.UiAccentColor) == "table" and decoded.UiAccentColor.Hex then
        ...
        if okAccent and accent then Library.Palette.Accent = accent end
    end
    if type(decoded.UiAccent2Color) == "table" and decoded.UiAccent2Color.Hex then
        ...
        if okAccent2 and accent2 then Library.Palette.Accent2 = accent2 end
    end
    ...
    if isfile and readfile and isfile(layoutPath) then
        local okLayout, layout = pcall(function() return HttpService:JSONDecode(readfile(layoutPath)) end)
        if okLayout and type(layout) == "table" then
            ...
            if layout.HotkeysX and layout.HotkeysY then Library.HotkeysPosition = Vector2.new(layout.HotkeysX, layout.HotkeysY) end
            if layout.StatsX and layout.StatsY then Library.StatsPosition = Vector2.new(layout.StatsX, layout.StatsY) end
        end
    end
    Config.SelectedConfig = Config.ConfigName
    SaveGlobalSettings()
...
LoadGlobalSettings()
if Config.AutoLoad then LoadNamedConfig(Config.SelectedConfig or Config.ConfigName) else ApplyThemePreset(Config.ThemePreset or "Nightfall", false) end

local WindowDrawings = {
    Shadow = MakeRoundedBox(Color3.new(0, 0, 0), 0.18), Outline = MakeRoundedBox(Library.Palette.Outline, 1),
    Background = MakeRoundedBox(Library.Palette.Background, 1), Topbar = MakeRoundedBox(Library.Palette.Sidebar, 1),
    Sidebar = MakeRoundedBox(Library.Palette.Sidebar, 1), Topline = MakeGradientLine(56),
    PatternBack = MakeStripePattern(72, 11), PatternFront = MakeStripePattern(72, 7),
...
local StatsDrawings = {
    Outline = MakeRoundedBox(Library.Palette.Outline, 1), Background = MakeRoundedBox(Library.Palette.Background, 1),
    Topline = MakeGradientLine(44), HeaderLine = CreateDrawing("Square", {Color = Library.Palette.Outline, Filled = true, Visible = false}),
    Row1 = MakeRoundedBox(Library.Palette.Element, 1), Row2 = MakeRoundedBox(Library.Palette.Element, 1),
    Row3 = MakeRoundedBox(Library.Palette.Element, 1), Title = CreateDrawing("Text", {Text = "BALL STATS", Color = Library.Palette.Text, Size = 12, Font = 2, Outline = true, Visible = false}),
...
local KeybindsDrawings = {
    Outline = MakeRoundedBox(Library.Palette.Outline, 1), Background = MakeRoundedBox(Library.Palette.Background, 1),
    Topline = MakeGradientLine(44), HeaderLine = CreateDrawing("Square", {Color = Library.Palette.Outline, Filled = true, Visible = false}),
    TitleIcon = CreateKeybindImageIcon(18), Title = CreateDrawing("Text", {Text = "HOTKEYS", Color = Library.Palette.Text, Size = 12, Font = 2, Outline = true, Visible = false}),
    Subtitle = CreateDrawing("Text", {Text = "", Color = Library.Palette.SubText, Size = 12, Font = 2, Outline = true, Visible = false}),
    Row1 = MakeRoundedBox(Library.Palette.Element, 1), Row2 = MakeRoundedBox(Library.Palette.Element, 1),
    Row3 = MakeRoundedBox(Library.Palette.Element, 1), Row4 = MakeRoundedBox(Library.Palette.Element, 1),
    ...
    MenuOutline = MakeRoundedBox(Library.Palette.Outline, 1), MenuBackground = MakeRoundedBox(Library.Palette.Background, 1),
    MenuOption1 = MakeRoundedBox(Library.Palette.Element, 1), MenuOption2 = MakeRoundedBox(Library.Palette.Element, 1),
...
function Library:CreateTab(tabName, tabIcon)
    local Tab = {
        Name = tabName, Icon = "", Sections = {}, Background = MakeRoundedBox(self.Palette.Hover, 0.88),
        ...
        Indicator = MakeRoundedBox(self.Palette.Accent, 1), CurrentColor = self.Palette.SubText,
        CurrentIconColor = self.Palette.SubText, BgAlpha = 0
    }

    function Tab:CreateSection(sectionName, sectionSide)
        local Section = {
            Name = sectionName, Side = sectionSide, Items = {}, Outline = MakeRoundedBox(Library.Palette.Outline, 1),
            Background = MakeRoundedBox(Library.Palette.Section, 1), PatternBack = MakeStripePattern(40, 9),
            PatternFront = MakeStripePattern(40, 5), Title = CreateDrawing("Text", {Text = sectionName, Color = Library.Palette.Text, Size = 12, Font = 2, Outline = true, Visible = false}),
        ...
        function Section:CreateToggle(name, flag, default)
            ...
            local Toggle = {
                Type = "Toggle", Height = 24, Flag = flag, BoxStroke = MakeRoundedBox(Library.Palette.Outline, 1),
                Box = MakeRoundedBox(Library.Palette.Element, 1), Fill = MakeRoundedBox(Library.Palette.Accent, 1),
                Label = CreateDrawing("Text", {Text = name, Size = 12, Font = 2, Outline = true, Visible = false}),
                CurrentColor = Library.Palette.SubText, Alpha = Config[flag] and 1 or 0, HitboxPos = Vector2.new(), HitboxSize = Vector2.new()
            }
        ...
        function Section:CreateSlider(name, flag, min, max, default, step)
            ...
            local Slider = {
                Type = "Slider", Height = 44, Min = min, Max = max, Step = step, Flag = flag,
                InputBuffer = FormatSliderValue(Config[flag], step), Label = CreateDrawing("Text", {Text = name, Color = Library.Palette.Text, Size = 12, Font = 2, Outline = true, Visible = false}),
                ValueLabel = CreateDrawing("Text", {Color = Library.Palette.Text, Size = 12, Font = 2, Outline = true, Visible = false}),
                Stroke = MakeRoundedBox(Library.Palette.Outline, 1), Background = MakeRoundedBox(Library.Palette.Element, 1),
                Fill = MakeRoundedBox(Library.Palette.Accent, 1), Knob = MakeRoundedBox(Library.Palette.Text, 1),
            ...
            function Slider:Update(x, y, w)
                ...
                if not isEditing then self.InputBuffer = FormatSliderValue(Config[self.Flag], self.Step) end
                local barY = y + 26
        ...
        function Section:CreateDropdown(name, flag, options, default)
            ...
            local Dropdown = {
                Type = "Dropdown", Height = 46, Options = options or {}, Flag = flag, IsOpen = false,
                ListHeight = 0, TargetListHeight = 0, OpenAlpha = 0, HoveredIndex = nil,
                ...
                ListStroke = MakeRoundedBox(Library.Palette.Outline, 1), ListBackground = MakeRoundedBox(Library.Palette.Element, 1),
                OptionDrawings = {}, OptionBounds = {}, ButtonPos = Vector2.new(), ButtonSize = Vector2.new(), ListPos = Vector2.new(), ListSize = Vector2.new()
            }
            ...
            function Dropdown:SetOptions(newOptions, preserveValue)
                ...
                if not preserveValue then
                    ...
                    for _, option in ipairs(self.Options) do
                        if tostring(option) == tostring(Config[self.Flag]) then found = true break end
                    end
                    if not found then
                        if #self.Options > 0 then Config[self.Flag] = self.Options[1] else Config[self.Flag] = nil end
                    end
            ...
            function Dropdown:GetVisibleOptionBounds()
                ...
                for index, bounds in ipairs(self.OptionBounds) do
                    if bounds and bounds.Visible then result[index] = bounds end
                end
                return result
            end
            ...
            function Dropdown:Update(x, y, w)
                ...
                for i, optionStr in ipairs(self.Options) do
                    ...
                    self.OptionBounds[i] = {Pos = optionPos, Size = optionSize, Visible = optionVisible}

                    if optionVisible and IsMouseInBounds(Library.Input.MousePos, optionPos, optionSize) then self.HoveredIndex = i end
                    ...
                    if optionVisible then
                        ...
                        if selectedIndex == i then txtDraw.Color = Library.Palette.Accent
                        elseif self.HoveredIndex == i then txtDraw.Color = Library.Palette.Text
                        else txtDraw.Color = Library.Palette.SubText end
                        txtDraw.Transparency = math.max(self.OpenAlpha, 0.08)
                    end
                end
                for i = #self.Options + 1, #self.OptionDrawings do self.OptionDrawings[i].Visible = false end
        ...
        function Section:CreateColorPicker(name, flag, defaultColor, defaultAlpha, callback)
            ...
            if initialAlpha == nil then initialAlpha = 1 end
            if type(Config[flag]) ~= "table" or not Config[flag].Color then
                Config[flag] = {Color = initialColor, Alpha = Clamp(initialAlpha, 0, 1)}
            end
            local h, s, v = Color3ToHSV(Config[flag].Color)

            local ColorPicker = {
                Type = "ColorPicker", Height = 36, Flag = flag, Callback = callback, IsOpen = false,
                Hue = h, Sat = s, Val = v, Alpha = Clamp(Config[flag].Alpha or 1, 0, 1),
                GridCols = 32, GridRows = 18, HueSteps = 32,
                Label = CreateDrawing("Text", {Text = name, Color = Library.Palette.Text, Size = 12, Font = 2, Outline = true, Visible = false}),
                Stroke = MakeRoundedBox(Library.Palette.Outline, 1), Background = MakeRoundedBox(Library.Palette.Element, 1),
                PreviewStroke = MakeRoundedBox(Library.Palette.Outline, 1), Preview = MakeRoundedBox(Config[flag].Color, 1),
                PreviewText = CreateDrawing("Text", {Color = Library.Palette.SubText, Size = 11, Font = 2, Outline = true, Visible = false}),
                PopupStroke = MakeRoundedBox(Library.Palette.Outline, 1), PopupBackground = MakeRoundedBox(Library.Palette.Background, 1),
                PopupSwatchStroke = MakeRoundedBox(Library.Palette.Outline, 1), PopupSwatch = MakeRoundedBox(Config[flag].Color, 1),
                PopupHex = CreateDrawing("Text", {Color = Library.Palette.Text, Size = 11, Font = 2, Outline = true, Visible = false}),
                SVStroke = MakeRoundedBox(Library.Palette.Outline, 1), HueStroke = MakeRoundedBox(Library.Palette.Outline, 1),
                SVGrid = CreateGridSquares(32 * 18), HueGrid = CreateGridSquares(32),
                ...
                ButtonPos = Vector2.new(), ButtonSize = Vector2.new(), PopupPos = Vector2.new(), PopupSize = Vector2.new(), SVPos = Vector2.new(), SVSize = Vector2.new(), HuePos = Vector2.new(), HueSize = Vector2.new()
            }
        ...
        function Section:CreateKeybind(name, flag, default)
            ...
            local Keybind = {
                Type = "Keybind", Height = 30, Flag = flag, Icon = CreateKeybindImageIcon(18),
                Label = CreateDrawing("Text", {Text = name, Color = Library.Palette.Text, Size = 12, Font = 2, Outline = true, Visible = false}),
                ButtonStroke = MakeRoundedBox(Library.Palette.Outline, 1), ButtonBox = MakeRoundedBox(Library.Palette.Element, 1),
        ...
        function Section:CreateButton(name, callback)
            local Button = {
                Type = "Button", Height = 34, Callback = callback, Stroke = MakeRoundedBox(Library.Palette.Outline, 1),
        ...
        function Section:CreateTextbox(name, flag, default)
            ...
            local Textbox = {
                Type = "Textbox", Height = 36, Flag = flag, Label = CreateDrawing("Text", {Text = name, Color = Library.Palette.Text, Size = 12, Font = 2, Outline = true, Visible = false}),
                Stroke = MakeRoundedBox(Library.Palette.Outline, 1), Background = MakeRoundedBox(Library.Palette.Element, 1),
                ValueLabel = CreateDrawing("Text", {Color = Library.Palette.SubText, Size = 12, Font = 2, Outline = true, Visible = false}),
            ...
            function Textbox:Update(x, y, w)
                ...
                if isActive and tick() % 1 < 0.5 then displayStr = displayStr .. "_" end
                self.ValueLabel.Visible = true
    ...
    if not Library.CurrentTab then Library.CurrentTab = Tab end
    return Tab
end
...
SecConfigsMain:CreateButton("Save Config", function()
    Config.SelectedConfig = Config.ConfigName
    SaveCurrentConfig()
    if ConfigListDropdown then ConfigListDropdown:SetOptions(GetAvailableConfigs(), true) end
end)
...
local OverlayMenuDrawings = {
    Outline = MakeRoundedBox(Library.Palette.Outline, 1), Background = MakeRoundedBox(Library.Palette.Background, 1),
    Option1 = MakeRoundedBox(Library.Palette.Element, 1), Option2 = MakeRoundedBox(Library.Palette.Element, 1),
...
local function CloseTransientUi()
    if Library.State.ActiveDropdown then Library.State.ActiveDropdown.IsOpen = false end
    if Library.State.ActiveColorPicker then Library.State.ActiveColorPicker.IsOpen = false end
    Library.State.ActiveDropdown = nil
    Library.State.ActiveSlider = nil
    if Library.State.ActiveSliderInput then CommitSliderInput(Library.State.ActiveSliderInput) end
...
local function HideHotkeysDrawings()
    ...
    }) do txt.Visible = false end
end

local function SafeHideDrawing(draw) if draw then draw.Visible = false end end

local function HideItem(item)
    if not item then return end
    if item.Type == "Toggle" then
        if item.BoxStroke then UpdateRoundedBox(item.BoxStroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.Box then UpdateRoundedBox(item.Box, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.Fill then UpdateRoundedBox(item.Fill, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        SafeHideDrawing(item.Label)
    elseif item.Type == "Slider" then
        if item.Stroke then UpdateRoundedBox(item.Stroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.Background then UpdateRoundedBox(item.Background, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.Fill then UpdateRoundedBox(item.Fill, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.ValueStroke then UpdateRoundedBox(item.ValueStroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.ValueBackground then UpdateRoundedBox(item.ValueBackground, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.KnobOuter then UpdateRoundedBox(item.KnobOuter, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.Knob then UpdateRoundedBox(item.Knob, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        SafeHideDrawing(item.Label) SafeHideDrawing(item.ValueLabel) SafeHideDrawing(item.Dot)
    elseif item.Type == "Dropdown" then
        item.IsOpen = false item.ListHeight = 0 item.TargetListHeight = 0 item.OpenAlpha = 0
        SafeHideDrawing(item.Label) SafeHideDrawing(item.ValueLabel) SafeHideDrawing(item.Icon)
        if item.Stroke then UpdateRoundedBox(item.Stroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.Background then UpdateRoundedBox(item.Background, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.ListStroke then UpdateRoundedBox(item.ListStroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.ListBackground then UpdateRoundedBox(item.ListBackground, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        for _, draw in ipairs(item.OptionDrawings or {}) do SafeHideDrawing(draw) end
    elseif item.Type == "ColorPicker" then
        SafeHideDrawing(item.Label) SafeHideDrawing(item.PreviewText) SafeHideDrawing(item.PopupHex)
        if item.Stroke then UpdateRoundedBox(item.Stroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.Background then UpdateRoundedBox(item.Background, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.PreviewStroke then UpdateRoundedBox(item.PreviewStroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.Preview then UpdateRoundedBox(item.Preview, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.PopupStroke then UpdateRoundedBox(item.PopupStroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.PopupBackground then UpdateRoundedBox(item.PopupBackground, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.PopupSwatchStroke then UpdateRoundedBox(item.PopupSwatchStroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.PopupSwatch then UpdateRoundedBox(item.PopupSwatch, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.SVStroke then UpdateRoundedBox(item.SVStroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.HueStroke then UpdateRoundedBox(item.HueStroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        HideGridSquares(item.SVGrid or {}) HideGridSquares(item.HueGrid or {})
        SafeHideDrawing(item.CursorOuter) SafeHideDrawing(item.CursorInner) SafeHideDrawing(item.HueLine) SafeHideDrawing(item.HueCapTop) SafeHideDrawing(item.HueCapBottom)
    elseif item.Type == "Keybind" then
        if item.Icon then UpdateKeybindImageIcon(item.Icon, Vec2(0, 0), Library.Palette.Accent, false) end
        SafeHideDrawing(item.Label) SafeHideDrawing(item.ValueLabel)
        if item.ButtonStroke then UpdateRoundedBox(item.ButtonStroke, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
        if item.ButtonBox then UpdateRoundedBox(item.ButtonBox, Vec2(0, 0), Vec2(0, 0), 0, Color3.new(0, 0, 0), 1, false) end
...
local function HideTabContent(tab)
    ...
    for _, section in ipairs(tab.Sections) do
        ...
        HideStripePattern(section.PatternBack) HideStripePattern(section.PatternFront)
        section.Title.Visible = false section.Line.Visible = false
        for _, item in ipairs(section.Items) do HideItem(item) end
    end
end
...
local function ForceHideMainUi()
    if HideWindowDrawings then HideWindowDrawings() end
    HideStatsDrawings() HideHotkeysDrawings()
    for _, tab in ipairs(Library.Tabs) do HideTabContent(tab) end
    CloseTransientUi()
    for _, tab in ipairs(Library.Tabs) do
        for _, section in ipairs(tab.Sections) do
            for _, item in ipairs(section.Items) do
    ...
    KeybindsDrawings.MenuOption1Text.Visible = false KeybindsDrawings.MenuOption2Text.Visible = false
    ...
    OverlayMenuDrawings.Option1Text.Visible = false OverlayMenuDrawings.Option2Text.Visible = false
end
...
local function ApplyHeldBackspace()
    ...
    if now < (Library.State.BackspaceNextRepeat or 0) then return end
...
HideWindowDrawings = function()
    ...
    HideGradientLine(WindowDrawings.Topline) HideStripePattern(WindowDrawings.PatternBack) HideStripePattern(WindowDrawings.PatternFront)
    WindowDrawings.SidebarLine.Visible = false WindowDrawings.TopBorder.Visible = false WindowDrawings.Title.Visible = false
    for _, tab in ipairs(Library.Tabs) do
        ...
        tab.Label.Visible = false tab.IconDraw.Visible = false HideTabContent(tab)
    end
end
...
local function DoClick()
    ...
    if Config.ParryMethod == "Click" then
        ...
        if keypress and keyrelease then
            keypress(0x46) keyrelease(0x46)
        end
    end
end

local AutoRuntime = {
    Ball = nil, LastBall = nil, LastBallPos = Vector3.zero, LastBallVel = Vector3.zero, LastBallSpeed = 0,
    LastTarget = nil, LastTargetChange = 0, Parries = 0, AutoSpam = false, Cooldown = false,
    WarpDetectedAt = -999, PingHistory = {}, SmoothedAccel = 0, LastAlive = false, Parried = false,
    TriggerParried = false, LastParryAt = 0, LastSpamAt = 0, LastManualClickAt = 0, TriggerScheduledAt = 0,
    LastTrainingParryAt = 0, LastTrainingBall = nil, CachedBall = nil, LastBallScanAt = 0, BallScanInterval = 0.06, LastSpawnScanAt = 0
}

local AutoSpamBatch = {Accumulator = 0, LastBatch = 0, TotalClicks = 0}

local function IsAutoSpamBatchActive()
    if not _G.MoonshadeActive then return false end
    if not AutoRuntime then return false end
    if not IsAliveMode() then return false end
    if not Config.AutoSpam or not AutoRuntime.AutoSpam then return false end
    if isrbxactive and not isrbxactive() then return false end
    local character = LocalPlayer.Character
...
local function UpdateAutoSpamBatch(dt)
    ...
    local clicksPerSecond = sensitivity * 24
    ...
    AutoRuntime.LastSpamAt = tick()
    
    local safeClicks = math.min(clicks, 3)
    for _ = 1, safeClicks do
        DoClick()
    end
end

RunService.RenderStepped:Connect(function()
    if not (isrbxactive and isrbxactive()) then return end
    ...
    for keyCode, keyName in pairs(KeyCodes) do
        ...
        if keyCode == 1 then isPressed = Library.Input.Mouse1Down elseif keyCode == 2 then isPressed = Library.Input.Mouse2Down end
        ...
        if isPressed and not Library.Input.KeysDown[keyCode] then
            ...
            elseif Library.State.ActiveSliderInput then
                ...
                if keyCode == 8 then slider.InputBuffer = string.sub(currentText, 1, -2)
                elseif keyCode == 13 then CommitSliderInput(slider) Library.State.ActiveSliderInput = nil
                elseif keyCode == 27 then slider.InputBuffer = FormatSliderValue(Config[slider.Flag], slider.Step) Library.State.ActiveSliderInput = nil
                else
                    local char = keyName
                    if keyCode >= 48 and keyCode <= 57 then slider.InputBuffer = currentText .. char
                    elseif keyCode >= 96 and keyCode <= 105 then slider.InputBuffer = currentText .. tostring(keyCode - 96)
                    elseif keyCode == 109 then if not string.find(currentText, "%-") then slider.InputBuffer = "-" .. currentText end
                    elseif keyCode == 189 then if not string.find(currentText, "%-") then slider.InputBuffer = "-" .. currentText end
                    elseif keyCode == 110 or keyCode == 190 then if not string.find(currentText, "%.") then slider.InputBuffer = currentText .. "." end
                    end
                end
            elseif Library.State.ActiveTextbox then
                local tbox = Library.State.ActiveTextbox
                local currentText = tostring(Config[tbox.Flag])
                if keyCode == 8 then Config[tbox.Flag] = string.sub(currentText, 1, -2)
                elseif keyCode == 13 or keyCode == 27 then Library.State.ActiveTextbox = nil
                elseif keyCode == 32 then Config[tbox.Flag] = currentText .. " " QueueSaveConfig()
                else
                    local char = keyName
                    if keyCode >= 65 and keyCode <= 90 then Config[tbox.Flag] = currentText .. (isShiftDown and char or string.lower(char)) QueueSaveConfig()
                    elseif keyCode >= 48 and keyCode <= 57 then Config[tbox.Flag] = currentText .. (isShiftDown and (ShiftModifiers[char] or char) or char) QueueSaveConfig()
                    end
                end
            else
                if keyName == NormalizeKeybindValue(Config.HideKeybind) then
                    if not Library.BindPressed then
                        ...
                        Library.ForceHidden = not Library.Visible
                        Library.BindPressed = true
                        if Library.ForceHidden then ForceHideMainUi() else CloseTransientUi() end
                    end
                elseif IsAliveMode() and keyName == NormalizeKeybindValue(Config.ParryKeybind) then HandleBindPress("ParryKeybind")
                elseif IsAliveMode() and keyName == NormalizeKeybindValue(Config.SpamKeybind) then HandleBindPress("SpamKeybind")
                elseif IsAliveMode() and keyName == NormalizeKeybindValue(Config.TriggerbotKeybind) then HandleBindPress("TriggerbotKeybind") end
            end
        elseif not isPressed then
            Library.Input.KeysDown[keyCode] = false
            if keyName == NormalizeKeybindValue(Config.HideKeybind) then Library.BindPressed = false
            elseif IsAliveMode() and keyName == NormalizeKeybindValue(Config.ParryKeybind) then HandleBindRelease("ParryKeybind")
            elseif IsAliveMode() and keyName == NormalizeKeybindValue(Config.SpamKeybind) then HandleBindRelease("SpamKeybind")
            elseif IsAliveMode() and keyName == NormalizeKeybindValue(Config.TriggerbotKeybind) then HandleBindRelease("TriggerbotKeybind") end
        end
    end
    ...
    local shouldShowHotkeys = Config.ShowHotkeyList and not Library.ForceHidden

    if not Library.Visible then
        Library.ForceHidden = true
        ForceHideMainUi()
    else
        Library.ForceHidden = false
    end
    ...
    if shouldShowHotkeys then
        ...
        if Library.Input.Mouse1Released then Library.State.HotkeysDragging = false end
        ...
        if rowBoxes[4] and HideRoundedBox then HideRoundedBox(rowBoxes[4]) else if rowBoxes[4] then rowBoxes[4].Visible = false end end
        if bindTexts[4] then bindTexts[4].Visible = false end
        if stateTexts[4] then stateTexts[4].Visible = false end
        if modeTexts[4] then modeTexts[4].Visible = false end
        ...
        for i, row in ipairs(hotkeyRows) do
            ...
            if row and row.Entry and row.Entry.BindFlag then
                ...
                local stateColumnW, modeColumnW = 36, 58
                ...
                if rowBox and HideRoundedBox then HideRoundedBox(rowBox) elseif rowBox then rowBox.Visible = false end
    ...
    if showStats then
        ...
        if Library.Input.Mouse1Released then Library.State.StatsDragging = false end
    ...
    if not Library.Visible then return end
    ...
    if Library.Input.Mouse1Clicked and IsMouseInBounds(Library.Input.MousePos, Vector2.new(posX, posY + 37), Vector2.new(150, sizeY - 37)) then
        ...
        for _, tab in ipairs(Library.Tabs) do
            if IsMouseInBounds(Library.Input.MousePos, Vector2.new(posX + 5, yOffset), Vector2.new(140, 32)) then
                ...
                CloseTransientUi()
                for _, t in ipairs(Library.Tabs) do HideTabContent(t) end
            end
            yOffset = yOffset + 36
        end
    end
        ...
        if Library.State.ActiveDropdown then
            ...
            if Library.Input.Mouse1Clicked then
                ...
                for _, section in ipairs(Library.CurrentTab.Sections) do
                    ...
                    if clickedDropdown then break end
                end
                ...
                if clickedDropdown then
                    ...
                    if pickedOption and dropdown.Options[pickedOption] ~= nil then
                        ...
                        if dropdown.Flag == "ThemePreset" then ApplyThemePreset(Config[dropdown.Flag], false) QueueSaveConfig()
                        elseif dropdown.Flag == "SelectedConfig" then Config.SelectedConfig = tostring(Config[dropdown.Flag])
                        else QueueSaveConfig() end
                        dropdown.IsOpen = false
                        Library.State.ActiveDropdown = nil
                        blockInput = true
                    else
    ...
    if not Library.Input.Mouse1Down then Library.State.ActiveColorDrag = nil end
    ...
    for _, tab in ipairs(Library.Tabs) do
        ...
        if not isCurrent then HideTabContent(tab) end
        ...
        if isCurrent then
            ...
            for _, section in ipairs(tab.Sections) do
                ...
                if section.Side == "Left" then leftY = itemY + 16 else rightY = itemY + 16 end
            end
        else
            for _, section in ipairs(tab.Sections) do
                SetVisible(section.Outline, false) SetVisible(section.Background, false)
                HideStripePattern(section.PatternBack) HideStripePattern(section.PatternFront)
                section.Title.Visible = false section.Line.Visible = false
                for _, item in ipairs(section.Items) do
                    ...
                    if item.OptionDrawings then
                        for _, optionDrawing in pairs(item.OptionDrawings) do optionDrawing.Visible = false end
                    end
...
local function Magnitude(v) return v and math.sqrt(v.X * v.X + v.Y * v.Y + v.Z * v.Z) or 0 end
local function Distance(a, b) return a and b and Magnitude(a - b) or 0 end
...
local function Flatten(v) return v and Vector3.new(v.X, 0, v.Z) or Vector3.zero end
    ...
    pcall(function()
        ...
        if pingObj then
            if memory_read and pingObj.Address then
                local ok, value = pcall(function() return memory_read("double", pingObj.Address + 0xC8) end)
                if ok and type(value) == "number" and value > 0 then currentPing = value else currentPing = tonumber(pingObj.Value) or currentPing end
            else
...
local function IsTargetingMe(targetName)
    local character = LocalPlayer.Character
    if character and character:FindFirstChild("Highlight") then return true end
    ...
    if target == myName or target == myDisplay then return true end
    local clean = string.gsub(target, '%.%.%.$', '')
    if #clean >= 3 then
        if string.sub(myName, 1, #clean) == clean or string.sub(myDisplay, 1, #clean) == clean then return true end
        if string.find(myName, clean, 1, true) or string.find(myDisplay, clean, 1, true) then return true end
    end
    return false
end
...
local function CheckIsSpam(params)
    ...
    if params.EntityDistance > range then return false, params.Parries end
    if params.BallDistance > range then return false, params.Parries end
    ...
    if params.BallDistance <= accuracy and params.Parries >= 5 then return true, params.Parries end
...
local function AnalyzeTrajectory(ballPosition, ballVelocity, playerPosition)
    ...
    if currentDot ~= currentDot then currentDot = 1 end
    return currentDot
end

local function DetectWarp(ballPosition, dt)
    if dt <= 0 then return false end
    if AutoRuntime.LastBallPos == Vector3.zero then return false end
    local velMag = Magnitude(AutoRuntime.LastBallVel)
    if velMag < 3 then return false end
...
local function GetParryThreshold(ballPosition, ballVelocity, ballSpeed, playerPosition, ping, dt)
    ...
    local dotValue = AnalyzeTrajectory(ballPosition, ballVelocity, playerPosition)
    ...
    local Ping_Max, Ping_Min = ping, ping
    ...
    if Ping_Jitter > 20 then Jitter_Buffer = (Ping_Jitter / 10) * Config.PingMultiplier * 1.5 end

    local Frame_Lag_Buffer = 0
    if dt > 0.03 then Frame_Lag_Buffer = ballSpeed * dt * 0.8 end
    ...
    if DetectWarp(ballPosition, dt) then AutoRuntime.WarpDetectedAt = tick() end
    local timeSinceWarp = tick() - AutoRuntime.WarpDetectedAt
    if timeSinceWarp < 0.55 then baseThreshold = baseThreshold + (6.0 * (1 - (timeSinceWarp / 0.55))) end
    ...
    if cappedSpeed > Curve_Min_Speed_Threshold and dotValue < Curve_Dot_Limit then
        isCurving = true
    end

    if isCurving then
        baseThreshold = math.clamp(ballDistance * 0.25, 8.0, 15.0)
    end
...
local function GetBallTargetName(ball)
    if not ball then return nil end
    local target = ball:GetAttribute("target")
    if target == nil then target = ball:GetAttribute("Target") end
    return target
end

local function GetBallFoldersForMode()
    if IsDeadMode() then return {Workspace:FindFirstChild("TrainingBalls")} end
    return {Workspace:FindFirstChild("Balls")}
end

local function IsValidBallCandidate(ball, deadMode)
    if not ball then return false end
    local parent = ball.Parent
    if not parent then return false end
    local targetName = GetBallTargetName(ball)
    if not IsTargetingMe(targetName) then return false end
    if deadMode then return parent.Name == "TrainingBalls" end
    return ball:GetAttribute("realBall") == true
end

local function GetBestBall()
    local deadMode = IsDeadMode()
    local now = tick()
    if AutoRuntime.CachedBall and IsValidBallCandidate(AutoRuntime.CachedBall, deadMode) then return AutoRuntime.CachedBall end
    if now - (AutoRuntime.LastBallScanAt or 0) < (AutoRuntime.BallScanInterval or 0.06) then return AutoRuntime.CachedBall end
    AutoRuntime.LastBallScanAt = now
    AutoRuntime.CachedBall = nil
    local folders = GetBallFoldersForMode()
    for _, folder in ipairs(folders) do
        if folder then
            local children = folder:GetChildren()
            for i = 1, #children do
                local ball = children[i]
                if IsValidBallCandidate(ball, deadMode) then
                    AutoRuntime.CachedBall = ball
                    return ball
                end
            end
        end
...
local function UpdateBallAutomation(dt)
    dt = dt or 0.016
    local character = LocalPlayer.Character
    local root = character and character:FindFirstChild("HumanoidRootPart")
    local humanoid = character and character:FindFirstChild("Humanoid")
    local aliveMode = IsAliveMode()
    local deadMode = IsDeadMode()
    ...
    if not root or not humanoid or (humanoid.Health <= 0 and not deadMode) then
        AutoRuntime.AutoSpam = false
    ...
    if ball ~= AutoRuntime.LastTrainingBall then
        AutoRuntime.LastTrainingBall = ball
        AutoRuntime.Parried = false
        AutoRuntime.LastTrainingParryAt = 0
    end
    if not ball then
        AutoRuntime.AutoSpam = false
    ...
    local isTrainingBall = trainingBallsFolder and ball:IsDescendantOf(trainingBallsFolder) or false
    
    local ping = GetRawPing()
    table.insert(AutoRuntime.PingHistory, ping)
    local pingHistoryLimit = math.max(5, math.floor(Config.PingSampleCount))
    local pingOverflow = #AutoRuntime.PingHistory - pingHistoryLimit
    if pingOverflow > 0 then
        for _ = 1, pingOverflow do table.remove(AutoRuntime.PingHistory, 1) end
    end
    ...
    local spamParams = {
        Speed = ballSpeed, Parries = AutoRuntime.Parries, BallDistance = RuntimeState.TargetDistance,
        EntityDistance = nearestDistance, Dot = dotValue, Ping = ping
    }
    
    local autoParryEnabled = aliveMode and Config.AutoParry
    local autoSpamEnabled = aliveMode and Config.AutoSpam
    local triggerbotEnabled = aliveMode and Config.TriggerbotEnabled
    local trainingBallsEnabled = deadMode and Config.LobbyParry

    if targetingMe and autoSpamEnabled then AutoRuntime.AutoSpam = CheckIsSpam(spamParams) else AutoRuntime.AutoSpam = false end

    local validTarget = (autoParryEnabled and targetingMe) or (trainingBallsEnabled and isTrainingBall and targetingMe)
    local canAutoParry = autoParryEnabled or trainingBallsEnabled
        ...
        if validTarget then
            ...
            if not Config.NoClickOnBallSpawn or sinceSpawn > 0.12 then
                if RuntimeState.TargetDistance <= threshold and ballSpeed >= Config.MinThreatSpeed then
                    if trainingBallsEnabled and isTrainingBall then
                        local trainingDelay = math.clamp((tonumber(Config.ParryCooldown) or 0.06), 0.035, 0.18)
                        if tick() - AutoRuntime.LastTrainingParryAt >= trainingDelay then
                            AutoRuntime.LastTrainingParryAt = tick()
                            AutoRuntime.Parried = false
                            ExecuteParryAction(true, dotValue, isCurving)
                        end
                    elseif not AutoRuntime.Parried then
                        AutoRuntime.Parried = true
                        ExecuteParryAction(false, dotValue, isCurving)
                    end
                elseif trainingBallsEnabled and isTrainingBall and RuntimeState.TargetDistance > threshold + 8 then
                    AutoRuntime.Parried = false
                end
            end
        else
...
local function UpdateBallSpawnState()
    local now = tick()
    if now - (AutoRuntime.LastSpawnScanAt or 0) < 0.2 then return end
    AutoRuntime.LastSpawnScanAt = now
    local ball = GetBestBall()
    if ball and RuntimeState.LastBallSpawn <= 0 then RuntimeState.LastBallSpawn = now
    elseif not ball then RuntimeState.LastBallSpawn = now end
end

RunService.Heartbeat:Connect(function(dt)
    UpdateAutoSpamBatch(dt)
    UpdateBallAutomation(dt)
    UpdateBallSpawnState()
end)
